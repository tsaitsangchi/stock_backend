"""
audit_source_availability.py v0.5 (Strict Source Availability Audit · §6.8.8-C Time-Drift Tolerance · §6.8.8-D Full-Market Mode · §0.4 Progress Heartbeat · §6.8.8-E Quota-Error Detection)
================================================================================
**最後更新日期**: 2026-05-22
**主權狀態**: ACTIVE (憲法 v6.0.0 §14.7-L 對齊 + §6.8.8-C 時點漂移容忍規則落地 + §14.7-AP 治權閉環 + §6.8.8-D 全市場驗證模式 + §14.7-AQ 範圍對稱性補齊 + §0.4 [Hybrid Observability] 進度心跳 + §6.8.8-E Quota-Error 分類 + §14.7-AR 錯誤分類治權補齊)
...
| :--- | :--- | :--- |
| **1. [標準執行：core+convex 150 嚴格驗證]** | `$ python scripts/maintenance/audit_source_availability.py --universe core --all --include-fred --strict` | audit_source_availability v0.5 |
| **2. [單一個股全表驗證]** | `$ python scripts/maintenance/audit_source_availability.py --id 2330 --all --strict` | audit_source_availability v0.5 |
| **3. [來源端 snapshot 寫出]** | `$ python scripts/maintenance/audit_source_availability.py --universe core --all --strict --snapshot-out /tmp/api_start_dates_core150.json` | audit_source_availability v0.5 |
| **4. [離線重跑：用既有 snapshot]** | `$ python scripts/maintenance/audit_source_availability.py --universe core --all --strict --snapshot-in /tmp/api_start_dates_core150.json` | audit_source_availability v0.5 |
| **5. [僅 FinMind（略 FRED）]** | `$ python scripts/maintenance/audit_source_availability.py --universe core --all --strict` | audit_source_availability v0.5 |
| **6. [§6.8.8-C 時點漂移容忍：預設 3 個日曆日]** | `$ python scripts/maintenance/audit_source_availability.py --universe core --all --include-fred --strict --drift-tolerance 3` | audit_source_availability v0.5 |
| **7. [§6.8.8-C 嚴格模式：相容 v0.1]** | `$ python scripts/maintenance/audit_source_availability.py --universe core --all --include-fred --strict --drift-tolerance 0` | audit_source_availability v0.5 |
| **8. [§6.8.8-D 全市場驗證：~2,798 支]** | `$ python scripts/maintenance/audit_source_availability.py --universe full --all --include-fred --strict --special-full-market-reason "DB rebuild bootstrap 2026-05-22 full-market validation"` | audit_source_availability v0.5 |
...
| **10. [§0.4 進度心跳：每 100 stock]** | `$ python scripts/maintenance/audit_source_availability.py --universe full --all --include-fred --strict --special-full-market-reason "<reason>" --progress-interval 100` | audit_source_availability v0.5 |
| **11. [§0.4 靜默模式：相容 v0.3]** | `$ python scripts/maintenance/audit_source_availability.py --universe core --all --include-fred --strict --progress-interval 0` | audit_source_availability v0.5 |
| **12. [§6.8.8-E quota-error 分類]** | quota 耗盡時 audit 自動歸為 `api_errors`（不入 mismatch）；報表 mismatch table 之 API_ERROR row 顯示 `error` 欄位含 raw msg | audit_source_availability v0.5 |
                ...
                for idx, sid in enumerate(stocks, start=1):
                    for ds in datasets:
                        ...
                        if row["status"] == "SOURCE_EMPTY_OK":
                            ...
                            # v0.5 §6.8.8-E: api_errors 已在 except 區塊計數，避免雙重
                            pass
                        elif row["status"] != "OK":
                            self.mismatch += 1
                ...
                if include_fred:
                    for series_id in FRED_SERIES:
                        ...
                        if row["status"] == "TIME_DRIFT_OK":
                            ...
                            # v0.5 §6.8.8-E: fred_api_errors 已在 except 區塊計數，避免雙重
                            pass
                        elif row["status"] != "OK":
