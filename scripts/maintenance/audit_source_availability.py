"""
audit_source_availability.py v0.8 (Strict Source Availability Audit · §6.8.8-C Time-Drift Tolerance · §6.8.8-D Full-Market Mode · §0.4 Progress Heartbeat · §6.8.8-E Quota-Error Detection · §14.7-AS Time-Drift Hotfix · §6.8.8-E.1 Transient Timeout Retry · §3.2A.I Parallel Workers · §14.7-DJ Pure-Generic Schema Source)
================================================================================
**最後更新日期**: 2026-05-23
**主權狀態**: ACTIVE (憲法 v6.1.0 §14.7-L 對齊 + §6.8.8-C 時點漂移容忍規則落地 + §14.7-AP 治權閉環 + §6.8.8-D 全市場驗證模式 + §14.7-AQ 範圍對稱性補齊 + §0.4 [Hybrid Observability] 進度心跳 + §6.8.8-E Quota-Error 分類 + §14.7-AR 錯誤分類治權補齊 + §14.7-AS `_is_time_drift_ok` 字串減法 latent bug hotfix + **§6.8.8-E.1 transient timeout retry (`[30s, 300s]` 退避)** + **§3.2A.I parallel workers (`--workers ≤ 4` 平行 audit;預期 8h+ → ~2-3h)** + **§14.7-DJ Pure-Generic：FinMind schema 來源改 `get_dataset_columns` + `FINMIND_PIPELINE_DATASETS`** + 對齊 §14.7-AU v6.1.0 升版)
...
8. **[Sovereignty Declaration]** (v0.2, 憲章 §3.2A 橫切稽核工具)：本程式為 §3.2A 橫切基礎設施稽核工具
   ...
   不持有 schema 定義；§14.7-DJ (pure-generic)：FinMind 表 schema 來源為 `get_dataset_columns()`
   （DB 即真理，generic auto-schema 所建），表清單為 `FINMIND_PIPELINE_DATASETS`；FredData/infra 仍走宣告 schema。
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| **v0.8** | 2026-06-08 | Codex | **§14.7-DJ Pure-Generic：schema 來源改 `get_dataset_columns` + `FINMIND_PIPELINE_DATASETS`**：FinMind 原始表已退役 `DATASET_REGISTRY`/`FINMIND_API_TABLES` schema 白名單（改 generic auto-schema 自動建表，DB 即真理）。**功能變更 4 點**：(I) import 由 `DATASET_REGISTRY, FINMIND_API_TABLES` 改為 `FINMIND_PIPELINE_DATASETS, get_dataset_columns`；(II) `STOCK_LEVEL_DATASETS` 由迭代 `FINMIND_API_TABLES`（排除 TaiwanStockInfo）改為 `list(FINMIND_PIPELINE_DATASETS)`（本即不含名冊）；(III) `_db_summary` 之 `DATASET_REGISTRY[dataset]["columns"]` 改 `get_dataset_columns(dataset)`，守門由 `dataset not in DATASET_REGISTRY` 改為「`get_dataset_columns` 空 dict（表未建）→ ValueError」；(IV) 核心定義 [Sovereignty Declaration] schema 引用源說明更新 + L2/L5 補 §14.7-DJ 摘要。**治權邊界嚴守**：verdict 計算 / CLI / 報告格式 / classify / time-drift / quota / 並行 / record_lifecycle 接線**全不變**；FredData/infra 仍走宣告 schema。 | **ACTIVE** |
...
try:
    ...
    from core.data_schema import FINMIND_PIPELINE_DATASETS, get_dataset_columns
...
MAX_WORKERS = 4
# §14.7-DJ (pure-generic):FinMind 表退役 DATASET_REGISTRY/FINMIND_API_TABLES 白名單,
# 改用 FINMIND_PIPELINE_DATASETS(10 張 per-stock 原始表,本即不含 TaiwanStockInfo 名冊)。
STOCK_LEVEL_DATASETS = list(FINMIND_PIPELINE_DATASETS)
...
class SourceAvailabilityAuditor:
    ...
    def _db_summary(self, stock_id, dataset, conn):
        # §14.7-DJ (pure-generic):欄位型別來源改 get_dataset_columns(DB 即真理);
        # 空 dict = 表尚未建立(generic auto-schema)→ 視為不可驗證。
        columns = get_dataset_columns(dataset)
        if not columns:
            raise ValueError(f"Dataset table not present in DB: {dataset}")
