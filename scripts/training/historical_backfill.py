...
def predict_for_date(
    ...
    for i, bdate in enumerate(biz_dates):
        ...
        trajectory.append({
            ...
            "price_q10":         float(p_q10),
            "price_q25":         float(p_q25),
            "price_q50":         float(p_q50),
            "price_q75":         float(p_q75),
            "price_q90":         float(p_q90),
            "ensemble_price":    float(ens_price),
            "current_close":     float(current_close),
            "prob_up":           float(round(prob_up, 4)),
            "confidence_level":  str(confidence),
            "model_agreement":   float(round(agreement, 4)),
            "xgb_prob":          float(xgb_prob_db),
            "lgb_prob":          float(lgb_prob_db),
            "tft_prob":          float(tft_prob_db) if tft_prob_db is not None else None,
            "extreme_valuation": bool(is_extreme),
            "macro_shock":       bool(is_macro),
            "is_backfill":       True,
        })
