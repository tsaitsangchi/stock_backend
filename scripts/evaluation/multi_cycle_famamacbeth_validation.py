"""
multi_cycle_famamacbeth_validation.py v0.1 (Multi-Cycle Fama-MacBeth Cross-Sectional Baseline · per §14.7-CY Tree Family Comparison Baseline · per CLAUDE.md §一.11 三段式入憲)
================================================================================
**最後更新日期**: 2026-06-04
**主權狀態**: MULTI-CYCLE 4-HORIZON FAMA-MACBETH CROSS-SECTIONAL BASELINE VALIDATION + §14.7-CY HORIZON-EXTENSION + §14.7-CX 8-YEAR OOS EXTENSION + §一.10 SOURCE-TRACEABLE (per CLAUDE.md §一.11 強制三段式標頭;§14.7-CY T_CY-1 system script 強制;§一.10 三類唯一 source 強制) + §14.7-DE/DF §0.0-I 雙 helper(get_canonical_panel_dates / summarize_horizon_metrics)
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用「Fama-MacBeth 橫斷面回歸」(經典資產定價方法) 在 37 個股票特徵上預測個股未來報酬、選最看好的前 20 支做多,作為「複雜模型(樹 / Transformer)到底有沒有贏過經典橫斷面線性方法」之共同比較基準。

**它怎麼做(步驟)**:
1. 取 397 支「乾淨核心股」+ 每股 37 個經驗證的特徵(估值、動能、籌碼、財務等;全來自真實 FinMind/FRED API,無 AI 亂補值)。
2. 把 2013-05 ~ 2026-06 切成多個月度時間點(panel)。
3. **逐點往前走(walk-forward)**:每個訓練 panel「橫斷面標準化」37 特徵,跑一條「未來報酬 ~ 特徵」的橫斷面 OLS(用 np.linalg.lstsq)→ 得到該期的係數向量;**Fama-MacBeth 係數估計 = 訓練窗內各期係數的平均**。
4. 預測下一期每支股票報酬 = 標準化後特徵 · 平均係數;依分數挑最看好的 **前 20 支等權做多**,跟「全市場平均」比,算這一期賺賠 —— 全程只用過去資料,不偷看未來(防資料洩漏)。
...
7. 用門檻(§14.7-CZ T_CZ-6:有效 t 值 ≥ 4.20 且 Sharpe ≥ 2.40 且 勝率 ≥ 79%)判定:**這橫斷面基準在哪個週期「真的能賺錢且可信」**。

**輸入**:資料庫(feature_values 特徵 + TaiwanStockPriceAdj 股價)。**輸出**:JSON(各週期成績)+ log。
**它不做的事**:不訓練正式上線模型、不改資料庫(純讀取評估;§3.1 evaluation 角色);僅用 numpy/scipy,不用 statsmodels。
**為什麼需要它**:回答「複雜模型有沒有贏過經典 Fama-MacBeth 橫斷面方法」的實證裁判;所有模型用同一支標準、同窗、同指標 → 成績才能公平並排比較。
...
8. **[Fama-MacBeth Cross-Sectional Baseline]** (v0.1, 憲法 §14.7-DG common comparison baseline): 本程式為 classic cross-sectional regression baseline(per-panel 橫斷面 OLS via np.linalg.lstsq → 訓練窗係數平均 = Fama-MacBeth 係數);numpy/scipy only(NO statsmodels);與 tree / transformer / linear 共享同一 walk-forward / overlap correction / honest annualization protocol。
9. **[Zero Hardcoded Verdict]** (v0.1, 憲法 §5.6.3): 4 horizons 之 significance verdict 動態計算(`abs(eff_t_stat) > 1.997`,由 helper 計),**不**硬編 ✅/❌ 結論。
10. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切模組): 本程式為 **§14.7-CY Multi-Cycle Validation Fama-MacBeth baseline 實作**(對標 multi_cycle_validation.py LGBM 為樹族第一實作)。**治權邊界**:(a) §3.2 evaluation 模組;(b) 五套禁令(§0.1-A / §0.2-A / §0.3-A / §0.0-E.4 / §6.8)不涉;(c) T1-T3 不分層;(d) §8.5 anti-leakage 不處理(features 已 anti-leakage compliant per feature_store_builder;coefficient mean 僅用過去 panels);(e) **不訓練 production model**(僅 evaluate / 不寫 model_registry);(f) **不修改 feature_values / TaiwanStockPriceAdj**(read-only);(g) 唯一職責:跑 4-horizon walk-forward Fama-MacBeth evaluation + 計算 metrics + 持久化 JSON。
...
> 本程式作為 §14.7-CY Multi-Cycle Fama-MacBeth baseline 實作,依「驗證面向」拆分為 5 大功能群;每群對應憲章治權契約。
...
### Group B. Walk-Forward Fama-MacBeth Estimation — internal per-horizon

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Expanding window | `for i in range(1, len(panel_keys))`(coeff mean over [0..i-1] → predict i)| §14.7-CW T_CW-2 |
| B.2 Per-panel cross-sectional OLS | `np.linalg.lstsq(X_std_with_intercept, y)` per train panel | §14.7-DG baseline |
| B.3 FM coefficient = mean per-panel coeff | `mean_coef = np.mean(coef_stack, axis=0)` | Fama-MacBeth (1973)|
| B.4 Predict = std features · mean coeff | `pred = X_test_std_with_intercept @ mean_coef` | anti-leakage |
...
| D.2 JSON persistence | `reports/multi_cycle_famamacbeth_<timestamp>.json` | §14.7-CY T_CY-1 |
| D.3 Meta context | `_meta`(tool_ver / numpy_version / seed / run_at)| §一.10 source_traceability |
...
| 場景 | 對應命令 |
| :--- | :--- |
| 日常 multi-cycle validation | `python scripts/evaluation/multi_cycle_famamacbeth_validation.py --dry-run` |
| Commit 評估結果 | `python scripts/evaluation/multi_cycle_famamacbeth_validation.py --commit --output reports/<path>.json` |
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-06-04 | Codex | **首版:§14.7-DG common comparison baseline — Fama-MacBeth 橫斷面 baseline 實作**(clone multi_cycle_xgboost_validation.py v0.2 結構,僅換模型 train/predict + 標頭)。**功能 4 點**:(a) 4-horizon walk-forward(5/20/60/252d);(b) per-panel 橫斷面標準化 + np.linalg.lstsq OLS → 訓練窗係數平均 = Fama-MacBeth 係數(numpy/scipy only,NO statsmodels);(c) 預測 = 標準化下一期特徵 · 平均係數 → top-20;(d) 共用 `get_canonical_panel_dates` / `summarize_horizon_metrics`(§14.7-DE/DF §0.0-I 單一引用源)。**治權邊界**:read-only(不寫 model_registry / 不改 feature_values);全 (b) DB query + source-pure(`is_null_imputed IS NOT TRUE`);0 AI memory(per §一.10)。**Output**:stdout + JSON(`reports/multi_cycle_famamacbeth_<timestamp>.json`)。**v0.1 治權邊界**:不入憲(無新 doctrine,僅 §14.7-DG baseline implementation)。 | **ACTIVE** |
"""
...
def _cross_sectional_standardize(X):
    """Cross-sectional z-score per feature column (per-panel). Zero-variance cols → 0."""
...
def _panel_coefficients(X, y):
    """Cross-sectional OLS of y on standardized features (+intercept) via np.linalg.lstsq.
    Returns coefficient vector of length (n_features + 1); intercept is the last element."""
    Xs = _cross_sectional_standardize(X)
...
def evaluate_horizon(cur, panels, horizon_days, universe, label):
    ...
    # Pre-compute per-panel Fama-MacBeth coefficients (cross-sectional OLS on each panel).
    panel_coef = {}
    for k in panel_keys:
        X_k, y_k, _, _ = panel_data[k]
        if len(X_k) < 2:
            continue
        y_k_w = winsorize(np.array(y_k))
        panel_coef[k] = _panel_coefficients(X_k, y_k_w)

    for i in range(1, len(panel_keys)):
        test_key = panel_keys[i]
        X_test, y_test, _, _ = panel_data[test_key]
        coef_stack = [panel_coef[panel_keys[j]] for j in range(i) if panel_keys[j] in panel_coef]
        if len(coef_stack) < 1:
            continue
        mean_coef = np.mean(np.vstack(coef_stack), axis=0)  # Fama-MacBeth coefficient = mean of per-panel coeffs

        Xs_test = _cross_sectional_standardize(X_test)
        A_test = np.hstack([Xs_test, np.ones((Xs_test.shape[0], 1))])
        pred_te = A_test @ mean_coef
...
def main():
    global SEED
    parser = argparse.ArgumentParser(description=f"Multi-Cycle Fama-MacBeth Validation {TOOL_VER}")
    ...
    parser.add_argument("--seed", type=int, default=SEED,
                        help="§一.10 #3 multi-run seed(canonical 5422;Fama-MacBeth 為 deterministic,seed 僅入 _meta)")
    args = parser.parse_args()
    if not args.dry_run and not args.commit: args.dry_run = True
    SEED = args.seed  # §一.10 #3: recorded in _meta (FM estimator is deterministic)
    ...
    logger.info("="*100)
    logger.info(f"Multi-Cycle Fama-MacBeth Validation {TOOL_VER}(per §14.7-CY / §14.7-DG)")
    logger.info("="*100)
    logger.info(f"  Horizons: {horizon_labels}")
    logger.info(f"  numpy ver: {np.__version__}")
    ...
    try:
        ...
        logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix(Fama-MacBeth)\n{'='*100}")
        ...
        if args.output or args.commit:
            output_path = args.output or f"reports/multi_cycle_famamacbeth_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            output_results["_meta"] = {
                "tool": "multi_cycle_famamacbeth_validation.py",
                "tool_ver": TOOL_VER,
                "model_family": "famamacbeth",
