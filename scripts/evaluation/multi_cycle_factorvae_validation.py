"""
multi_cycle_factorvae_validation.py v0.1 (FactorVAE · VAE Dynamic Factor Model · AAAI 2022 / Duan et al. · Multi-Cycle Stock-Price Validation · per CLAUDE.md §一.11 三段式)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: FactorVAE(VAE DYNAMIC FACTOR MODEL / AAAI 2022)4-HORIZON WALK-FORWARD VALIDATION + §14.7-CY HORIZON-DOCTRINE 第二族(neural)+ §14.7-DC v0.18 SOURCE-PURE UNIVERSE + §一.10 SOURCE-TRACEABLE(全 DB)+ §一.10 #3 MULTI-RUN + 共同比較基準(COMMON COMPARISON BASELINE)新增實作(FactorVAE)+ §一.11 三段式合規 + §14.7-DE §0.0-I panel-date helper
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 FactorVAE(變分自編碼器動態因子模型) 序列模型,吃每支股票的「歷史價格序列」,預測未來報酬,評估「靠它選股能不能賺錢、準不準、可不可信」。
...
1. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切分析模組): 本程式為 evaluation 後處理工具,非序列落地鏈模組。
   **治權邊界**:(a) §3.2 evaluation 模組;(b) 不涉五套禁令(§0.1/§0.2/§0.3);(c) T1-T3 不分層;
   ...
   (g) 唯一職責:FactorVAE 4-horizon walk-forward 預測 + 共同比較基準 metrics + JSON 持久化。
2. **[Common Comparison Baseline]** (v0.1, reports/common_model_comparison_baseline_v1.md): 本程式為共同比較基準之**新增實作**(FactorVAE)—
   universe v0.18(398 source-pure)× 資料驅動 monthly panels × 真實 forward log returns(TaiwanStockPriceAdj)× 4 horizons(5/20/60/252d)×
   top-20 equal-weight long × 0.6% cost × {Sharpe, Win, Eff-t, T_CZ-6 gate}。與 iTransformer / 全 tree 模型套用**完全相同** protocol +
   **完全相同** realized targets → 精準度(precision)/ 信任度(trust)比較 apples-to-apples。模型用各自 natural representation
   (tree=38 cross-sectional features;iTransformer=跨股多變量 weekly return matrix;FactorVAE=**跨股 weekly return matrix → VAE 潛在因子**),
...
5. **[Real FactorVAE — VAE Dynamic Factor Model]** (v0.1, Duan/Wang/Chen/Xu, AAAI 2022 "FactorVAE: A Probabilistic Dynamic Factor
   Model Based on Variational Autoencoder for Predicting Cross-Sectional Stock Returns"): 核心為 **VAE 動態因子模型** —— 每股 lookback return 序列經
   per-stock feature extractor 編為個股潛在特徵 e_i,factor encoder q(z|x) 聚合橫斷面(mean over valid stocks)+ 個股特徵 → 產生 K 個潛在因子之
   mu_z / logvar_z(reparameterize z = mu_z + eps·exp(0.5·logvar_z));factor decoder 由 e_i 產生 per-stock factor loadings beta_i,
   predicted future = head over (beta_i · z) → project 至未來 S weeks。非 surrogate;從頭以 torch.nn 實作。
   **訓練 loss 額外含 KL 項**:masked_MSE(pred,Y) + beta_kl · KL(N(mu_z,var_z)||N(0,1))(KL 對 latent 平均)。
   eval/predict 模式用 mu_z(不抽樣)且僅回傳 [B,N,S]。variable-#variates 由 key_padding_mask 處理(橫斷面聚合 mask 掉 padded stocks)。
6. **[Point-Forecast → Calibration N/A]** (v0.1): FactorVAE 之 predicted future 為 point forecast(MSE + KL loss),非 quantile 模型 →
...
9. **[§一.10 #3 Multi-Run]** (v0.1): stochastic(torch init / dropout / sgd shuffle / VAE reparameterize 抽樣)→ 須 ≥3 seeds {5422,7331,1009};
...
### Group B. FactorVAE Model(跨股 weekly return matrix → VAE 潛在因子 → forecast)

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Return matrix | load_return_matrix() → daily close → calendar-week → log-return matrix[week × stock]| §一.10 / §一.13 source-pure |
| B.2 Model | FactorVAE:per-stock extractor + factor encoder q(z|x)(mu_z/logvar_z)+ factor decoder(loadings beta_i)+ head(d→S)| Duan et al. AAAI 2022 |
| B.3 Train | train_factorvae() → Adam + masked-MSE + beta_kl·KL(N(mu_z,var_z)||N(0,1))+ grad-clip(CPU)| VAE factor model |
| B.4 Predict | predict_forward() → mu_z(no sampling)→ forecast 未來 S weekly returns → cumsum → 4-horizon scores | §8.5 leakage-safe |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 JSON | --output reports/factorvae_v0/<...>.json(schema-compatible + precision/trust)| §一.10 / §二.4 |
...
| 場景 | 命令 |
| :--- | :--- |
| Smoke(plumbing 驗證)| `python scripts/evaluation/multi_cycle_factorvae_validation.py --smoke --output reports/factorvae_v0/_smoke.json` |
| 完整單 seed | `... --seed 5422 --output reports/factorvae_v0/fvae_s5422.json` |
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-06-02 | Codex | **首版**:FactorVAE(VAE Dynamic Factor Model / AAAI 2022 / Duan et al.)multi-cycle 股價預測驗證 + 共同比較基準新增實作。跨股 weekly return matrix → per-stock feature extractor → factor encoder q(z|x)(K 個潛在因子 mu_z/logvar_z + reparameterize)→ factor decoder(per-stock loadings beta_i)→ head 投影未來 S weeks → 4-horizon cumsum scores。訓練 loss = masked-MSE + beta_kl·KL(N(mu_z,var_z)||N(0,1))。eval 用 mu_z 不抽樣僅回傳 [B,N,S]。與 baseline 同 universe(v0.18/398)/ panels(§14.7-DE 資料驅動)/ forward returns(TaiwanStockPriceAdj)/ portfolio(top-20)/ cost(0.6%)/ gate(T_CZ-6)。precision(rank-IC / dir-acc / RMSE / MAE / R²)+ trust(Eff-t / 多 seed;calibration N/A 因 point-forecast)。§一.10 全 DB source-traceable;§一.10 #3 multi-seed;§8.5 leakage-safe by lookback/forecast split;§一.11 三段式合規。 | ACTIVE |
"""
...
# ════════════════════════════════════════════════════════════════════════════
# Group B — FactorVAE return-matrix construction (variates = stocks)
# ════════════════════════════════════════════════════════════════════════════
...
# ── FactorVAE architecture (from-scratch torch.nn; VAE dynamic factor model) ──
def _build_model(L, S, d_model, n_factors, dropout):
    import torch
    ...
    class FactorVAE(nn.Module):
        def __init__(self):
            super().__init__()
            # per-stock feature extractor: each stock's L-series → latent feature e_i
            self.extractor = nn.Sequential(
                nn.Linear(L, d_model), nn.GELU(), nn.Dropout(dropout),
                nn.Linear(d_model, d_model),
            )
            # factor encoder q(z|x): cross-section aggregate + per-stock feature → K factors mu/logvar
            self.enc = nn.Sequential(nn.Linear(2 * d_model, d_model), nn.GELU(), nn.Dropout(dropout))
            self.mu_head = nn.Linear(d_model, n_factors)
            self.logvar_head = nn.Linear(d_model, n_factors)
            # factor decoder: per-stock factor loadings beta_i = Linear(e_i) → [B,N,K]
            self.loading = nn.Linear(d_model, n_factors)
            # head: (beta_i · z) scalar → future S weekly returns
            self.head = nn.Linear(1, S)

        def forward(self, x, key_padding_mask=None):    # x:[B,N,L]  mask:[B,N] True=ignore
            e = self.extractor(x)                       # [B,N,d_model] per-stock latent features
            if key_padding_mask is not None:
                valid = (~key_padding_mask).unsqueeze(-1).float()   # [B,N,1] 1=valid
            else:
                valid = torch.ones(e.shape[0], e.shape[1], 1, dtype=e.dtype, device=e.device)
            csum = (e * valid).sum(dim=1)                            # [B,d_model]
            cnt = valid.sum(dim=1).clamp(min=1.0)                    # [B,1]
            agg = (csum / cnt).unsqueeze(1).expand(-1, e.shape[1], -1)  # [B,N,d_model] cross-section mean
            h = self.enc(torch.cat([e, agg], dim=-1))                # [B,N,d_model]
            hpool = (h * valid).sum(dim=1) / cnt                     # [B,d_model] pooled over valid stocks
            mu_z = self.mu_head(hpool)                               # [B,K]
            logvar_z = self.logvar_head(hpool)                       # [B,K]
            if self.training:
                eps = torch.randn_like(mu_z)
                z = mu_z + eps * torch.exp(0.5 * logvar_z)           # reparameterize
            else:
                z = mu_z                                             # eval: no sampling
            beta = self.loading(e)                                   # [B,N,K] per-stock factor loadings
            score = (beta * z.unsqueeze(1)).sum(dim=-1, keepdim=True)  # [B,N,1] beta_i · z
            pred = self.head(score)                                  # [B,N,S]
            if self.training:
                return pred, mu_z, logvar_z
            return pred                                              # eval: only [B,N,S]

    return FactorVAE()


def train_factorvae(ret_mat, week_cut, L, S, seed, epochs, lr, d_model, n_factors,
                    batch_size, dropout, beta_kl, min_valid=N_TOP + 5):
    """Train FactorVAE on all windows whose lookback+target ∈ weeks ≤ week_cut (leakage-safe).

    loss = masked_MSE(pred, Y) + beta_kl · KL(N(mu_z,var_z)||N(0,1))  (KL averaged over the latent).
    Returns (model, mu, sigma) — global standardization stats from past returns (NOT VAE latent);
    ...
    model = _build_model(L, S, d_model, n_factors, dropout)
    ...
    for _ep in range(epochs):
        ...
        for bs in range(0, len(usable), batch_size):
            ...
            pred, mu_z, logvar_z = model(X, key_padding_mask=kpm)  # [B,N,S], [B,K], [B,K]
            w = (lbv & tgv).unsqueeze(-1).float()                  # only complete-lookback & complete-target tokens
            l = lossf(pred, Y) * w
            mse = l.sum() / w.sum().clamp(min=1.0)
            kl = -0.5 * torch.mean(1 + logvar_z - mu_z.pow(2) - logvar_z.exp())  # KL(N(mu_z,var_z)||N(0,1))
            loss = mse + beta_kl * kl
...
def predict_forward(model, mu, sigma, ret_mat, week_cut, L, S, cols):
    ...
    Point forecast (eval mode: mu_z, no sampling) → no quantiles → weekly calibration N/A (empty)."""
    import torch
...
def aggregate_horizon(label, horizon_days, panel_top_rets, panel_univ_rets,
    ...
    return {
        ...
        "calibration_p10_p90_coverage": calib_cov,  # None for FactorVAE (point forecast, non-quantile)
    }
...
def run(args):
    ...
    logger.info(f"FactorVAE refit at panel indices {refit_points} (every {refit_every}); "
                f"L={L}wk S={S}wk d_model={args.d_model} n_factors={args.n_factors} beta_kl={args.beta_kl} "
    ...
    for pi, as_of in enumerate(panels):
        ...
        if pi in refit_points or model is None:
            tr0 = time.monotonic()
            res = train_factorvae(ret_mat, wc, L, S, args.seed, args.epochs, args.lr,
                                   args.d_model, args.n_factors, args.batch_size, args.dropout, args.beta_kl)
            if res is None:
...
def main():
    ap = argparse.ArgumentParser(description=f"Multi-Cycle FactorVAE Validation {TOOL_VER}")
    ...
    ap.add_argument("--n-factors", type=int, default=8)
    ...
    logger.info("=" * 100)
    logger.info(f"Multi-Cycle FactorVAE Validation {TOOL_VER} (VAE Dynamic Factor Model / AAAI 2022)")
    ...
    logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix (FactorVAE)\n{'='*100}")
    ...
    if args.output:
        ...
        out["_meta"] = {
            "tool": "multi_cycle_factorvae_validation.py", "tool_ver": TOOL_VER,
            "model": "FactorVAE (VAE dynamic factor model, 2022, from-scratch torch.nn)",
            ...
            "d_model": args.d_model, "n_factors": args.n_factors, "beta_kl": args.beta_kl,
            "refit_every": args.refit_every, "epochs": args.epochs, "smoke": args.smoke,
            "variates": "stocks (cross-sectional → VAE latent factors)",
