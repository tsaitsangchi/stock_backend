"""
multi_cycle_timesnet_validation.py v0.1 (TimesNet · 2D Temporal Variation · ICLR 2023 / Wu et al. · Multi-Cycle Stock-Price Validation · per CLAUDE.md §一.11 三段式)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: TimesNet(2D TEMPORAL VARIATION / ICLR 2023)4-HORIZON WALK-FORWARD VALIDATION + §14.7-CY HORIZON-DOCTRINE 神經族 + §14.7-DC v0.18 SOURCE-PURE UNIVERSE + §一.10 SOURCE-TRACEABLE(全 DB)+ §一.10 #3 MULTI-RUN + 共同比較基準(COMMON COMPARISON BASELINE)新增實作 + §一.11 三段式合規 + §14.7-DE §0.0-I panel-date helper 單一引用源
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 TimesNet(把一維時間序列拆成多個週期、攤成二維圖、用卷積看「週期內 × 週期間」變化的序列模型),吃每支股票的「歷史價格序列」,預測未來報酬,評估「靠它選股能不能賺錢、準不準、可不可信」。
...
1. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切分析模組): 本程式為 evaluation 後處理工具,非序列落地鏈模組。
   **治權邊界**:(a) §3.2 evaluation 模組;(b) 不涉五套禁令(§0.1/§0.2/§0.3);(c) T1-T3 不分層;
   ...
   (g) 唯一職責:TimesNet 4-horizon walk-forward 預測 + 共同比較基準 metrics + JSON 持久化。
2. **[Common Comparison Baseline]** (v0.1, reports/common_model_comparison_baseline_v1.md): 本程式為共同比較基準之**新增實作 (TimesNet)**(同族第一為 multi_cycle_tft_validation.py)—
   ...
   **完全相同** realized targets → 精準度(precision)/ 信任度(trust)比較 apples-to-apples。模型用各自 natural representation
   (tree=38 cross-sectional features;TFT=每股 weekly 序列 + GroupNormalizer pooled;TimesNet=**每股 weekly return 序列 channel-independent → FFT 2D 週期攤平**),
...
5. **[Real TimesNet — 2D Temporal Variation]** (v0.1, Wu/Hu/Liu/Wang/Long, ICLR 2023 "TimesNet: Temporal 2D-Variation
   Modeling for General Time Series Analysis"): 核心 = 將 **1D 時間序列經 FFT 取 top-k 主導頻率 → 對應週期 p,把長度攤成 [週期數 × 週期長] 之 2D map**,
   以 2D Inception 卷積同時建模「週期內(intraperiod)× 週期間(interperiod)」變化,再依 amplitude softmax 加權各週期分支求和 + residual。
   非 surrogate;從頭以 torch.nn 實作(value embed Linear + TimesBlock(rfft + 2D Inception conv)+ project Linear),與 multi_cycle_transformer_dedicated_validation.py 同慣例。
   variable-#variates 採 channel-independent([B,N,L] → reshape [B*N,L])→ 每股序列獨立處理;key_padding_mask 結構上忽略(由 loss-mask 上游處理)。
6. **[Point-Forecast → Calibration N/A]** (v0.1): TimesNet 原始為 point forecast(MSE loss),非 quantile 模型 →
...
### Group B. TimesNet Model(每股 weekly return 序列 → FFT 2D 週期攤平 → forecast)

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Return matrix | load_return_matrix() → daily close → calendar-week → log-return matrix[week × stock]| §一.10 / §一.13 source-pure |
| B.2 Model | TimesNet:value-embed(L→d)+ TimesBlock(rfft top-k 週期 → 2D Inception conv)+ project(L*d→S)| Wu et al. ICLR 2023 |
| B.3 Train | train_timesnet() → Adam + MSE(masked)+ grad-clip(CPU)| channel-independent |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 JSON | --output reports/timesnet_v0/<...>.json(schema-compatible + precision/trust)| §一.10 / §二.4 |
...
| 場景 | 命令 |
| :--- | :--- |
| Smoke(plumbing 驗證)| `python scripts/evaluation/multi_cycle_timesnet_validation.py --smoke --output reports/timesnet_v0/_smoke.json` |
| 完整單 seed | `... --seed 5422 --output reports/timesnet_v0/tnet_s5422.json` |
...
# ════════════════════════════════════════════════════════════════════════════
# Group B — TimesNet return-matrix construction (channel-independent per stock)
# ════════════════════════════════════════════════════════════════════════════
...
# ── TimesNet architecture (from-scratch torch.nn; 2D temporal-variation via FFT) ──
def _build_model(L, S, d_model, top_k, n_blocks, dropout):
    import torch
    import torch.nn as nn
    import torch.nn.functional as F

    class InceptionBlock2d(nn.Module):
        """Small 2D Inception: a couple of Conv2d with different kernels, GELU."""
        def __init__(self, c):
            super().__init__()
            self.b1 = nn.Conv2d(c, c, kernel_size=1)
            self.b3 = nn.Conv2d(c, c, kernel_size=3, padding=1)
            self.b5 = nn.Conv2d(c, c, kernel_size=5, padding=2)
            self.act = nn.GELU()

        def forward(self, x):                                # x:[BN, c, P, p]
            return self.act(self.b1(x) + self.b3(x) + self.b5(x))
    ...
    class TimesBlock(nn.Module):
        ...
        def forward(self, h):                                # h:[BN, L, d]
            BN, T, C = h.shape
            xf = torch.fft.rfft(h, dim=1)                    # [BN, T//2+1, d]
            amp = xf.abs().mean(dim=2)                        # [BN, T//2+1] mean amplitude over d
            amp[:, 0] = 0.0                                   # drop DC component
            k = min(top_k, amp.shape[1] - 1)
            if k < 1:
                return h
            weights, freqs = torch.topk(amp, k, dim=1)        # [BN, k] each
            # global dominant periods (shared across batch via mean amplitude) → reshape-safe
            gamp = amp.mean(dim=0)                             # [T//2+1]
            _, gfreqs = torch.topk(gamp, k)                   # [k]
            res = torch.zeros_like(h)
            wsoft = torch.softmax(weights, dim=1)             # [BN, k] amplitude weights
            for i in range(k):
                f = int(gfreqs[i].item())
                period = max(int(T // f) if f > 0 else T, 1)
                period = min(period, T)
                npad = (period - (T % period)) % period
                hp = F.pad(h, (0, 0, 0, npad)) if npad else h  # pad time axis → [BN, T+npad, d]
                Tp = hp.shape[1]
                rows = Tp // period
                m = hp.reshape(BN, rows, period, C).permute(0, 3, 1, 2)  # [BN, d, rows, period]
                m = self.inception(m)                          # 2D conv over (interperiod × intraperiod)
                m = m.permute(0, 2, 3, 1).reshape(BN, Tp, C)[:, :T, :]   # back → [BN, T, d]
                res = res + m * wsoft[:, i].view(BN, 1, 1)      # amplitude-weighted branch
            return res + h                                     # residual

    class TimesNet(nn.Module):
        def __init__(self):
            super().__init__()
            self.embed = nn.Linear(1, d_model)                 # value embed: each time-step scalar → d_model
            self.blocks = nn.ModuleList([TimesBlock() for _ in range(n_blocks)])
            self.norm = nn.LayerNorm(d_model)
            self.drop = nn.Dropout(dropout)
            self.head = nn.Linear(L * d_model, S)              # flattened temporal head → future S weekly returns

        def forward(self, x, key_padding_mask=None):           # x:[B,N,L]  mask:[B,N] (ignored; loss-masked upstream)
            B, N, T = x.shape
            h = x.reshape(B * N, T, 1)                         # channel-independent: each series separate
            h = self.embed(h)                                  # [BN, L, d]
            for blk in self.blocks:
                h = self.norm(blk(h))
            h = self.drop(h)
            out = self.head(h.reshape(B * N, T * d_model))     # [BN, S]
            return out.reshape(B, N, S)                        # [B,N,S]

    return TimesNet()


def train_timesnet(ret_mat, week_cut, L, S, seed, epochs, lr, d_model, top_k, n_blocks,
                   batch_size, dropout, min_valid=N_TOP + 5):
    """Train TimesNet on all windows whose lookback+target ∈ weeks ≤ week_cut (leakage-safe).
    ...
    model = _build_model(L, S, d_model, top_k, n_blocks, dropout)
...
def aggregate_horizon(label, horizon_days, panel_top_rets, panel_univ_rets,
    ...
    return {
        ...
        "calibration_p10_p90_coverage": calib_cov,  # None for TimesNet (point forecast, non-quantile)
    }
...
def run(args):
    ...
    logger.info(f"TimesNet refit at panel indices {refit_points} (every {refit_every}); "
                f"L={L}wk S={S}wk d_model={args.d_model} top_k={args.top_k} n_blocks={args.n_blocks} "
    ...
    for pi, as_of in enumerate(panels):
        ...
        if pi in refit_points or model is None:
            tr0 = time.monotonic()
            res = train_timesnet(ret_mat, wc, L, S, args.seed, args.epochs, args.lr,
                                 args.d_model, args.top_k, args.n_blocks, args.batch_size, args.dropout)
            if res is None:
...
def main():
    ap = argparse.ArgumentParser(description=f"Multi-Cycle TimesNet Validation {TOOL_VER}")
    ...
    logger.info("=" * 100)
    logger.info(f"Multi-Cycle TimesNet Validation {TOOL_VER} (2D Temporal Variation / ICLR 2023)")
    ...
    logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix (TimesNet)\n{'='*100}")
    ...
    if args.output:
        ...
        out["_meta"] = {
            "tool": "multi_cycle_timesnet_validation.py", "tool_ver": TOOL_VER,
            "model": "TimesNet (2D temporal variation, ICLR 2023, from-scratch torch.nn)",
            ...
            "d_model": args.d_model, "top_k": args.top_k, "n_blocks": args.n_blocks,
            "refit_every": args.refit_every, "epochs": args.epochs, "smoke": args.smoke,
            "variates": "stocks (channel-independent, 2D temporal variation)",
