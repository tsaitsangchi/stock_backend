"""
multi_cycle_ipca_validation.py v0.1 (Multi-Cycle IPCA Instrumented-PCA Validation Engine · per §14.7-CY Cross-Sectional Latent-Factor Extension · per CLAUDE.md §一.11 三段式入憲)
================================================================================
**最後更新日期**: 2026-06-04
**主權狀態**: MULTI-CYCLE 4-HORIZON IPCA (Instrumented PCA, Kelly-Pruitt-Su 2019, numpy-only ALS) VALIDATION + §14.7-CY HORIZON-EXTENSION + §14.7-CX 8-YEAR OOS EXTENSION + §一.10 SOURCE-TRACEABLE (per CLAUDE.md §一.11 強制三段式標頭;§一.10 三類唯一 source 強制) + §14.7-DE/DF §0.0-I 雙 helper 單一引用源(get_canonical_panel_dates / summarize_horizon_metrics)
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 IPCA 隱因子模型(Instrumented PCA,Kelly-Pruitt-Su 2019 的橫斷面學術 SOTA) 在 37 個股票特徵上預測「每支股票未來會漲多少」,挑最看好的 top-20 等權做多,並給出「賺多少 / 準不準 / 可不可信」的成績單。

**它怎麼做(步驟)**:
1. 取 397 支「乾淨核心股」+ 每股 37 個經驗證的特徵(估值、動能、籌碼、財務等;全來自真實 FinMind/FRED API,無 AI 亂補值)。
2. 把 2013-05 ~ 2026-06 切成多個月度時間點(panel)。
3. **逐點往前走(walk-forward)**:每個時間點「只用那之前的資料」訓練 IPCA —— IPCA 認為「每支股票的因子曝險 β 不是隨便估,而是由它當下的 37 個特徵 Z 透過一張共用映射表 Γ(37×K)算出來」,再用 β 去解釋報酬。模型靠「交替最小平方(ALS)」反覆估 Γ 與每期的因子報酬 f,直到收斂。
4. 預測「之後」每支股票的期望報酬 = Z·Γ·(訓練窗內因子報酬的平均 f̄),不偷看未來(防資料洩漏)。
5. 依預測分數挑最看好的 **前 20 支等權做多**,跟「全市場平均」比,算這一期賺賠。
6. 上面在 **4 種持有期**各做一遍:週(5 天)/ 月(20 天)/ 季(60 天)/ 年(252 天)。
7. 算每種週期的成績:**報酬率、Sharpe、勝率、預測命中率、跨次穩定度(可信度)、扣成本後年化報酬**(全經共用 helper `summarize_horizon_metrics`,與其他模型同指標 → 可比)。
8. 用門檻(§14.7-CZ T_CZ-6:有效 t 值 ≥ 4.20 且 Sharpe ≥ 2.40 且 勝率 ≥ 79%)判定:**這模型在哪個週期「真的能賺錢且可信」**。

**輸入**:資料庫(feature_values 特徵 + TaiwanStockPriceAdj 股價)。**輸出**:JSON(各週期成績)+ log。
**它不做的事**:不訓練正式上線模型、不改資料庫(純讀取評估;§3.2 evaluation 角色)。
**為什麼需要它**:回答「橫斷面隱因子(學術 SOTA)在這 37 特徵上能不能選股賺錢」的實證裁判;所有模型用同一支標準、同窗、同指標 → 成績才能公平並排比較(common baseline)。
...
8. **[Instrumented-PCA Cross-Sectional Model]** (v0.1, 憲法 §14.7-CY; Kelly-Pruitt-Su 2019 JFE): 本程式為 §14.7-CY 比較模型庫之橫斷面隱因子 SOTA(IPCA)。模型 `r_{i,t+1} ≈ β_{i,t}' f_{t+1}`,loadings `β_{i,t} = Z_{i,t} Γ`(Z = 37 standardized characteristics;Γ = 37×K 映射;K=5 latent factors);numpy-only alternating least squares(ALS)估計。非 tree、非 sequence/foundation model。
...
11. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 evaluation 模組): 本程式為 **§14.7-CY Multi-Cycle Validation 之 IPCA 隱因子實作**。**治權邊界**:(a) §3.2 evaluation 模組;(b) 五套禁令(§0.1-A / §0.2-A / §0.3-A / §0.0-E.4 / §6.8)不涉;(c) T1-T3 不分層;(d) §8.5 anti-leakage 由 walk-forward train-on-past-only(Γ、f̄ 僅由過去 panels 估)保證(features 已 anti-leakage compliant per feature_store_builder);(e) **不訓練 production model**(僅 evaluate / 不寫 model_registry);(f) **不修改 feature_values / TaiwanStockPriceAdj**(read-only);(g) 唯一職責:跑 4-horizon walk-forward IPCA evaluation + 計算 metrics + 持久化 JSON。
...
> 本程式作為 §14.7-CY Multi-Cycle 比較模型庫之 IPCA 橫斷面隱因子實作,依「驗證面向」拆分為 5 大功能群;每群對應憲章治權契約。
...
### Group B. Walk-Forward IPCA ALS Training — internal per-horizon

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Expanding window training | `for i in range(1, len(panel_keys))`(train [0..i-1] → test i)| §14.7-CY T_CY-2 |
| B.2 ALS factor step(given Γ → f_t)| `_ipca_fit()` step (a): cross-sectional OLS r_t on Z_t·Γ | KPS 2019 ALS |
| B.3 ALS gamma step(given {f_t} → Γ)| `_ipca_fit()` step (b): stacked-vec OLS, reshape L×K, orthonormalize | KPS 2019 ALS |
| B.4 Predict via Z·Γ·f̄ | `_ipca_predict()`: expected return = Z·Γ·mean(f_t) | KPS 2019 conditional E[r] |
| B.5 Per-panel z-score standardize Z | `_standardize_cols()`(fit per panel, no look-ahead)| anti-leakage |
...
| D.2 JSON persistence | `reports/multi_cycle_ipca_<timestamp>.json` | §14.7-CY T_CY-1 |
| D.3 Meta context | `_meta`(tool_ver / numpy_version / K / iters / seed / run_at)| §一.10 source_traceability |
...
| 場景 | 對應命令 |
| :--- | :--- |
| 日常 multi-cycle validation | `python scripts/evaluation/multi_cycle_ipca_validation.py --dry-run` |
| Commit 評估結果 | `python scripts/evaluation/multi_cycle_ipca_validation.py --commit --output reports/<path>.json` |
| 自訂 horizons | `... --horizons 7,30,90,365` |

### 不提供之旗標 (Intentionally Omitted)

- `--cost-per-rebal X`:0.6% 為 §14.7-CY T_CY-5 conservative standard(由 helper 固定)。
- `--n-factors K` / `--als-iters`:K=5 latent factors + ALS 20 iters 為模組常數(module constants),固定以利跨模型可比。
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-06-04 | Codex | **首版:§14.7-CY Multi-Cycle 比較模型庫之 IPCA 橫斷面隱因子實作**(Instrumented PCA, Kelly-Pruitt-Su 2019, numpy-only ALS)。**功能 4 點**:(a) 4-horizon walk-forward(5/20/60/252d);(b) ALS 估計 Γ(37×K=5)+ 每期因子 f_t(交替最小平方,20 iters,orthonormalize Γ);(c) §14.7-DE 單一引用源 panel 窗 + §14.7-DF 單一計算源 metric;(d) 397 core + `is_null_imputed IS NOT TRUE` source-pure。**模型**:loadings β=Z·Γ;factor step = 各 train 期 cross-sectional OLS r_t on Z_t·Γ;gamma step = stacked-vec OLS r on (f_t ⊗ Z_{i,t}) reshape L×K;predict = Z·Γ·mean(f_t)。**治權邊界**:read-only(不寫 model_registry / 不改 feature_values);全 (b) DB query;0 AI memory(per §一.10)。**Output**:stdout + JSON(`reports/multi_cycle_ipca_<timestamp>.json`)。clone 自 multi_cycle_xgboost_validation.py v0.2,**僅替換模型**(GBM → IPCA),其餘 universe query / panel 窗 / forward returns / top-20 / cost / T_CZ-6 / metric helper / CLI 全保持一致(common baseline)。 | **ACTIVE** |
"""
...
# IPCA module constants (Kelly-Pruitt-Su 2019). Kept fixed for cross-model comparability.
IPCA_N_FACTORS = 5          # K latent factors
IPCA_ALS_ITERS = 20         # alternating least squares iterations
IPCA_RIDGE = 1e-6           # ridge stabilizer for least-squares inverses
DEFAULT_SEED = 5422         # only seeds Gamma initialization (ALS is otherwise deterministic)
...
def _standardize_cols(Z):
    """Cross-sectional z-score each characteristic column within one panel (no look-ahead)."""
    mu = Z.mean(axis=0)
    sd = Z.std(axis=0)
...
def _ipca_fit(train_panels, n_factors, n_iters, seed):
    """Instrumented PCA via alternating least squares (Kelly-Pruitt-Su 2019).

    train_panels = list of (Z_t, r_t): Z_t = (N_t × L) standardized characteristics,
                   r_t = (N_t,) realized forward returns for that period.
    Model: r_{i,t} ≈ (Z_{i,t}' Γ) f_t, with Γ (L × K).
      step (a) given Γ: for each t, f_t = OLS of r_t on B_t = Z_t Γ.
      step (b) given {f_t}: vec(Γ) = OLS of stacked r_{i,t} on (f_t ⊗ Z_{i,t}); reshape (L × K), orthonormalize cols.
    Returns Γ (L × K) and f_bar = mean_t f_t over the training window.
    """
    L = train_panels[0][0].shape[1]
    K = n_factors
    rng = np.random.default_rng(seed)
    Gamma = rng.standard_normal((L, K))
    Gamma, _ = np.linalg.qr(Gamma)  # orthonormal init

    f_list = [np.zeros(K) for _ in train_panels]
    for _ in range(n_iters):
        # ----- step (a): given Gamma, estimate factor realization f_t per period (cross-sectional OLS) -----
        for ti, (Z_t, r_t) in enumerate(train_panels):
            B_t = Z_t @ Gamma                       # (N_t × K) instrumented loadings
            A = B_t.T @ B_t + IPCA_RIDGE * np.eye(K)
            f_list[ti] = np.linalg.solve(A, B_t.T @ r_t)

        # ----- step (b): given {f_t}, estimate Gamma by stacked-vec least squares -----
        # r_{i,t} = vec(Gamma)' (f_t ⊗ Z_{i,t}); accumulate normal equations over all (i,t).
        LK = L * K
        XtX = np.zeros((LK, LK))
        Xtr = np.zeros(LK)
        for (Z_t, r_t), f_t in zip(train_panels, f_list):
            # design rows = kron(f_t, Z_{i,t}) for each i  ->  (N_t × LK)
            Phi = np.kron(f_t.reshape(1, K), Z_t)       # (N_t × (K*L)) col-order: factor-major, char-minor
            XtX += Phi.T @ Phi
            Xtr += Phi.T @ r_t
        XtX += IPCA_RIDGE * np.eye(LK)
        vecG = np.linalg.solve(XtX, Xtr)
        # vecG is ordered factor-major (kron(f, Z)) -> reshape to (K, L) then transpose to (L, K)
        Gamma = vecG.reshape(K, L).T
        # identification: orthonormalize Gamma columns (resolves rotational indeterminacy, stabilizes ALS)
        Gamma, _ = np.linalg.qr(Gamma)

    # final factor pass with converged Gamma
    for ti, (Z_t, r_t) in enumerate(train_panels):
        B_t = Z_t @ Gamma
        A = B_t.T @ B_t + IPCA_RIDGE * np.eye(K)
        f_list[ti] = np.linalg.solve(A, B_t.T @ r_t)

    f_bar = np.mean(np.vstack(f_list), axis=0) if f_list else np.zeros(K)
    return Gamma, f_bar


def _ipca_predict(Z_test, Gamma, f_bar):
    """Conditional expected return E[r_{i,t+1}] = Z_{i,t}' Γ f̄  (per-stock score)."""
    return (Z_test @ Gamma) @ f_bar
...
def evaluate_horizon(cur, panels, horizon_days, universe, label, seed=DEFAULT_SEED):
    ...
    for i in range(1, len(panel_keys)):
        ...
        # build standardized (Z_t, r_t) training panels from all past panels (anti-leakage)
        train_panels = []
        n_train_rows = 0
        for j in range(i):
            X_j, y_j, _, _ = panel_data[panel_keys[j]]
            Z_j = _standardize_cols(np.array(X_j, dtype=float))
            r_j = winsorize(np.array(y_j, dtype=float))
            if len(r_j) >= IPCA_N_FACTORS:
                train_panels.append((Z_j, r_j))
                n_train_rows += len(r_j)
        if n_train_rows < 100 or not train_panels:
            continue

        Gamma, f_bar = _ipca_fit(train_panels, IPCA_N_FACTORS, IPCA_ALS_ITERS, seed)

        Z_te = _standardize_cols(np.array(X_test, dtype=float))
        pred_te = _ipca_predict(Z_te, Gamma, f_bar)
...
def main():
    parser = argparse.ArgumentParser(description=f"Multi-Cycle IPCA Validation {TOOL_VER}")
    ...
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED,
                        help="§一.10 #3 multi-run seed(canonical 5422;只影響 Γ 初始化,ALS 餘為 deterministic)")
    args = parser.parse_args()
    if not args.dry_run and not args.commit: args.dry_run = True
    seed = args.seed  # §一.10 #3: run seed threaded into Gamma init via evaluate_horizon
    ...
    logger.info("="*100)
    logger.info(f"Multi-Cycle IPCA Validation {TOOL_VER}(per §14.7-CY)")
    logger.info("="*100)
    logger.info(f"  Horizons: {horizon_labels}")
    logger.info(f"  numpy ver: {np.__version__} | K={IPCA_N_FACTORS} | ALS iters={IPCA_ALS_ITERS}")
    ...
    try:
        ...
        for label, days in horizon_labels:
            r = evaluate_horizon(cur, panels, days, universe, label, seed)
            if r: results[label] = r

        logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix(IPCA)\n{'='*100}")
        ...
        if args.output or args.commit:
            output_path = args.output or f"reports/multi_cycle_ipca_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            output_results["_meta"] = {
                "tool": "multi_cycle_ipca_validation.py",
                "tool_ver": TOOL_VER,
                "model_family": "ipca",
                "numpy_version": np.__version__,
                "n_factors": IPCA_N_FACTORS,
                "als_iters": IPCA_ALS_ITERS,
