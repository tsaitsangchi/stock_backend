"""
multi_cycle_moderntcn_validation.py v0.1 (ModernTCN · Modern Temporal Conv Network · ICLR 2024 / Luo & Wang · Multi-Cycle Stock-Price Validation · per CLAUDE.md §一.11 三段式)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: ModernTCN(MODERN TEMPORAL CONV NETWORK / ICLR 2024)4-HORIZON WALK-FORWARD VALIDATION + §14.7-CY HORIZON-DOCTRINE 第二族(neural)+ §14.7-DC v0.18 SOURCE-PURE UNIVERSE + §一.10 SOURCE-TRACEABLE(全 DB)+ §一.10 #3 MULTI-RUN + 共同比較基準(COMMON COMPARISON BASELINE)新增實作 + §一.11 三段式合規 + §14.7-DE §0.0-I panel-date helper
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 ModernTCN(現代化時序卷積網路) 序列模型,吃每支股票的「歷史價格序列」,預測未來報酬,評估「靠它選股能不能賺錢、準不準、可不可信」。
...
1. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切分析模組): 本程式為 evaluation 後處理工具,非序列落地鏈模組。
   **治權邊界**:(a) §3.2 evaluation 模組;(b) 不涉五套禁令(§0.1/§0.2/§0.3);(c) T1-T3 不分層;
   ...
   (g) 唯一職責:ModernTCN 4-horizon walk-forward 預測 + 共同比較基準 metrics + JSON 持久化。
2. **[Common Comparison Baseline]** (v0.1, reports/common_model_comparison_baseline_v1.md): 本程式為共同比較基準之**新增實作**(第一為 multi_cycle_tft_validation.py)—
   ...
   **完全相同** realized targets → 精準度(precision)/ 信任度(trust)比較 apples-to-apples。模型用各自 natural representation
   (tree=38 cross-sectional features;TFT=每股 weekly 序列 + GroupNormalizer pooled;ModernTCN=**channel-independent 每股 weekly return 序列卷積**),
...
5. **[Real ModernTCN — Modernized Convolution]** (v0.1, Luo & Wang, ICLR 2024 "ModernTCN: A Modern Pure Convolution Structure
   for General Time Series Analysis"): channel-independent 將 **每支股票之 length-L lookback return 序列** 視為單一通道時序,
   經 input stem(1→C channels)+ 堆疊 K 個 ModernTCN block(每 block = large-kernel DEPTHWISE Conv1d + BatchNorm + pointwise ConvFFN
   1×1 C→C·ffn→GELU→C·ffn→C,residual)+ temporal head Linear(L→S) → project 至未來 S weeks。
   非 surrogate;從頭以 torch.nn 實作(input Linear stem + depthwise/pointwise Conv1d blocks + project Linear),與 multi_cycle_transformer_dedicated_validation.py 同慣例。
   variable-#variates 由 loss masking 處理(channel-independent 結構天然容許不定 stock 數;模組本身可忽略 key_padding_mask)。
6. **[Point-Forecast → Calibration N/A]** (v0.1): ModernTCN 原始為 point forecast(MSE loss),非 quantile 模型 →
...
12. **[Idempotency]** (v0.1): 不寫 DB(--output 寫 JSON);可重跑;refit cadence / channels / kernel / n_blocks / epochs / lookback 全可配置 → 跑期可控。
...
| A.2 Panels | get_canonical_panel_dates() → data-driven monthly mid-month | §14.7-DE / §0.0-I 單一引用源 |
...
### Group B. ModernTCN Model(channel-independent 每股 weekly return 序列卷積 → forecast)

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Return matrix | load_return_matrix() → daily close → calendar-week → log-return matrix[week × stock]| §一.10 / §一.13 source-pure |
| B.2 Model | ModernTCN:input stem(1→C)+ K×(depthwise Conv1d + BN + pointwise ConvFFN, residual)+ project(L→S)| Luo & Wang ICLR 2024 |
| B.3 Train | train_moderntcn() → Adam + MSE(masked)+ grad-clip(CPU)| modernized convolution |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 JSON | --output reports/moderntcn_v0/<...>.json(schema-compatible + precision/trust)| §一.10 / §二.4 |
...
| 場景 | 命令 |
| :--- | :--- |
| Smoke(plumbing 驗證)| `python scripts/evaluation/multi_cycle_moderntcn_validation.py --smoke --output reports/moderntcn_v0/_smoke.json` |
| 完整單 seed | `... --seed 5422 --output reports/moderntcn_v0/mtcn_s5422.json` |
...
# ════════════════════════════════════════════════════════════════════════════
# Group B — ModernTCN return-matrix construction (channel-independent per stock)
# ════════════════════════════════════════════════════════════════════════════
...
# ── ModernTCN architecture (from-scratch torch.nn; faithful modernized convolution) ──
def _build_model(L, S, channels, kernel_size, n_blocks, dropout):
    import torch.nn as nn

    class ModernTCNBlock(nn.Module):
        """One ModernTCN block: large-kernel depthwise Conv1d + BatchNorm + pointwise ConvFFN, residual."""
        def __init__(self):
            super().__init__()
            pad = kernel_size // 2                            # 'same' padding for odd kernel
            self.dwconv = nn.Conv1d(channels, channels, kernel_size,
                                    padding=pad, groups=channels)  # depthwise: per-channel temporal mixing
            self.bn = nn.BatchNorm1d(channels)
            ffn = channels * 4
            self.pw1 = nn.Conv1d(channels, ffn, 1)           # pointwise 1x1 C→C·ffn
            self.act = nn.GELU()
            self.pw2 = nn.Conv1d(ffn, channels, 1)           # pointwise 1x1 C·ffn→C
            self.drop = nn.Dropout(dropout)

        def forward(self, z):                                # z:[*,C,L]
            r = z
            z = self.bn(self.dwconv(z))                      # depthwise + norm
            z = self.pw2(self.act(self.pw1(z)))              # ConvFFN
            return r + self.drop(z)                          # residual

    class ModernTCN(nn.Module):
        """ModernTCN (Luo & Wang, ICLR 2024). Channel-independent: same weights across every
        stock's length-L series. forward(x:[B,N,L]) → reshape [B*N,1,L] → conv stack → [B,N,S]."""
        def __init__(self):
            super().__init__()
            self.stem = nn.Linear(1, channels)               # input embed: scalar return → C channels
            self.blocks = nn.ModuleList([ModernTCNBlock() for _ in range(n_blocks)])
            self.head = nn.Linear(L, S)                      # temporal head: collapse length L → future S
            self.proj = nn.Linear(channels, 1)               # channel pool → scalar per step

        def forward(self, x, key_padding_mask=None):         # x:[B,N,L]  mask ignored (loss-masked upstream)
            B, N, L = x.shape
            z = x.reshape(B * N, L).unsqueeze(-1)             # [B*N,L,1] channel-independent series
            z = self.stem(z).transpose(1, 2)                 # [B*N,C,L]
            for blk in self.blocks:
                z = blk(z)                                   # [B*N,C,L]
            z = self.head(z)                                 # [B*N,C,S]
            out = self.proj(z.transpose(1, 2)).squeeze(-1)   # [B*N,S]
            return out.reshape(B, N, -1)                     # [B,N,S]

    return ModernTCN()


def train_moderntcn(ret_mat, week_cut, L, S, seed, epochs, lr, channels, kernel_size, n_blocks,
                    batch_size, dropout, min_valid=N_TOP + 5):
    """Train ModernTCN on all windows whose lookback+target ∈ weeks ≤ week_cut (leakage-safe).
    ...
    model = _build_model(L, S, channels, kernel_size, n_blocks, dropout)
    ...
    for _ep in range(epochs):
        ...
        for bs in range(0, len(usable), batch_size):
            ...
            kpm = ~lbv                                             # True = padded stock (loss-masked)
...
def aggregate_horizon(label, horizon_days, panel_top_rets, panel_univ_rets,
    ...
    return {
        ...
        "calibration_p10_p90_coverage": calib_cov,  # None for ModernTCN (point forecast, non-quantile)
    }
...
def run(args):
    ...
    logger.info(f"ModernTCN refit at panel indices {refit_points} (every {refit_every}); "
                f"L={L}wk S={S}wk channels={args.channels} kernel={args.kernel_size} n_blocks={args.n_blocks} "
    ...
    for pi, as_of in enumerate(panels):
        ...
        if pi in refit_points or model is None:
            tr0 = time.monotonic()
            res = train_moderntcn(ret_mat, wc, L, S, args.seed, args.epochs, args.lr,
                                  args.channels, args.kernel_size, args.n_blocks, args.batch_size, args.dropout)
            if res is None:
...
def main():
    ap = argparse.ArgumentParser(description=f"Multi-Cycle ModernTCN Validation {TOOL_VER}")
    ...
    ap.add_argument("--kernel-size", type=int, default=13, help="large depthwise kernel for ModernTCN block")
    ...
    logger.info("=" * 100)
    logger.info(f"Multi-Cycle ModernTCN Validation {TOOL_VER} (Modern Temporal Conv Network / ICLR 2024)")
    ...
    logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix (ModernTCN)\n{'='*100}")
    ...
    if args.output:
        ...
        out["_meta"] = {
            "tool": "multi_cycle_moderntcn_validation.py", "tool_ver": TOOL_VER,
            "model": "ModernTCN (Modern Temporal Conv Network, ICLR 2024, from-scratch torch.nn)",
            ...
            "channels": args.channels, "kernel_size": args.kernel_size, "n_blocks": args.n_blocks,
            "refit_every": args.refit_every, "epochs": args.epochs, "smoke": args.smoke,
            "variates": "stocks (channel-independent convolution)",
