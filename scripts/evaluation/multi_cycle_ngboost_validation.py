"""
multi_cycle_ngboost_validation.py v0.1 (Multi-Cycle NGBoost Probabilistic-GBM Validation Engine · per §14.7-DG common comparison baseline · per CLAUDE.md §一.11 三段式入憲)
================================================================================
**最後更新日期**: 2026-06-04
**主權狀態**: MULTI-CYCLE 4-HORIZON NGBOOST VALIDATION + §14.7-CY HORIZON-EXTENSION + §14.7-CX 8-YEAR OOS EXTENSION + §9.1 PROBABILISTIC CONTRACT + §一.10 SOURCE-TRACEABLE (per CLAUDE.md §一.11 強制三段式標頭;§14.7-CY T_CY-1 system script 強制;§一.10 三類唯一 source 強制) + §14.7-DE/DF §0.0-I 雙 helper(get_canonical_panel_dates / summarize_horizon_metrics)
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 NGBoost(機率梯度提升) 模型在 37 個股票特徵上預測個股未來報酬 —— 它不只給一個點估計,而是給「整條報酬機率分布(平均 + 標準差)」 —— 依預測「平均報酬」挑最看好的前 20 支做多,作為共同比較基準之一。

**它怎麼做(步驟)**:
1. 取 397 支「乾淨核心股」+ 每股 37 個經驗證的特徵(估值、動能、籌碼、財務等;全來自真實 FinMind/FRED API,無 AI 亂補值)。
2. 把 2013-05 ~ 2026-06 切成多個月度時間點(panel)。
3. **逐點往前走(walk-forward)**:每個時間點「只用那之前的資料」先用 StandardScaler 標準化特徵,再 fit 一個 NGBoost(機率梯度提升)模型,預測「之後」每支股票的報酬機率分布 —— 不偷看未來(防資料洩漏)。
4. **以預測平均報酬為主**(headline):依平均報酬挑最看好的 **前 20 支等權做多**,跟「全市場平均」比,算這一期賺賠;(機率分布之預測標準差另存 JSON `_meta` 供參考,top-20 排序只用平均)。
...
**輸入**:資料庫(feature_values 特徵 + TaiwanStockPriceAdj 股價)。**輸出**:JSON(各週期成績;_meta 含預測 std 統計)+ log。
**它不做的事**:不訓練正式上線模型、不改資料庫(純讀取評估;§3.2 evaluation 角色)。
**為什麼需要它**:NGBoost 直接 fit 報酬「機率分布」,天然對應 §9.1 機率預測契約;所有模型用同一支標準、同窗、同指標 → 成績才能公平並排比較。
...
8. **[Probabilistic GBM Baseline]** (v0.1, 憲法 §14.7-DG common comparison baseline / §9.1 機率預測契約): 本程式為 probabilistic gradient-boosting baseline(NGBoost Dist=Normal on standardized features),直接 fit 報酬機率分布(mean + std);top-20 ranking 用 mean prediction;與 tree / linear / transformer / foundation 共享同一 walk-forward / overlap correction / honest annualization protocol。
9. **[Zero Hardcoded Verdict]** (v0.1, 憲法 §5.6.3): 4 horizons 之 significance verdict 動態計算(`abs(eff_t_stat) > 1.997`,由 helper 計),**不**硬編 ✅/❌ 結論。
10. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切模組): 本程式為 **§14.7-CY Multi-Cycle Validation NGBoost probabilistic-GBM baseline 實作**(對標 multi_cycle_validation.py LGBM 為樹族第一實作)。**治權邊界**:(a) §3.2 evaluation 模組;(b) 五套禁令(§0.1-A / §0.2-A / §0.3-A / §0.0-E.4 / §6.8)不涉;(c) T1-T3 不分層;(d) §8.5 anti-leakage 不處理(features 已 anti-leakage compliant per feature_store_builder;train 僅用過去 panels);(e) **不訓練 production model**(僅 evaluate / 不寫 model_registry);(f) **不修改 feature_values / TaiwanStockPriceAdj**(read-only);(g) 唯一職責:跑 4-horizon walk-forward NGBoost evaluation + 計算 metrics + 持久化 JSON。
...
> 本程式作為 §14.7-CY Multi-Cycle NGBoost probabilistic-GBM baseline 實作,依「驗證面向」拆分為 5 大功能群;每群對應憲章治權契約。
...
### Group B. Walk-Forward NGBoost Training — internal per-horizon

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Expanding window training | `for i in range(1, len(panel_keys))`(train [0..i-1] → test i)| §14.7-CW T_CW-2 |
| B.2 StandardScaler + NGBRegressor(Dist=Normal, seed=5422)| `StandardScaler` + `NGBRegressor`(300 estimators / lr 0.03)| §14.7-DG baseline / §9.1 |
...
| D.2 JSON persistence | `reports/multi_cycle_ngboost_<timestamp>.json` | §14.7-CY T_CY-1 |
| D.3 Meta context | `_meta`(tool_ver / ngboost_version / seed / run_at / predicted std stats)| §一.10 source_traceability |
...
| 場景 | 對應命令 |
| :--- | :--- |
| 日常 multi-cycle validation | `python scripts/evaluation/multi_cycle_ngboost_validation.py --dry-run` |
| Commit 評估結果 | `python scripts/evaluation/multi_cycle_ngboost_validation.py --commit --output reports/<path>.json` |
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-06-04 | Codex | **首版:§14.7-DG common comparison baseline — NGBoost probabilistic-GBM baseline 實作**(clone multi_cycle_xgboost_validation.py v0.2 結構,僅換模型 train/predict + 標頭)。**功能 4 點**:(a) 4-horizon walk-forward(5/20/60/252d);(b) StandardScaler + NGBRegressor(Dist=Normal / 300 estimators / lr 0.03 / random_state=seed),top-20 用 `.predict()` mean;(c) 預測 std(`.pred_dist().scale`)統計存 JSON `_meta`(top-20 排序不用);(d) 共用 `get_canonical_panel_dates` / `summarize_horizon_metrics`(§14.7-DE/DF §0.0-I 單一引用源)。**治權邊界**:read-only(不寫 model_registry / 不改 feature_values);全 (b) DB query + source-pure(`is_null_imputed IS NOT TRUE`);0 AI memory(per §一.10)。**Output**:stdout + JSON(`reports/multi_cycle_ngboost_<timestamp>.json`)。**v0.1 治權邊界**:不入憲(無新 doctrine,僅 §14.7-DG baseline implementation)。 | **ACTIVE** |
"""
...
def evaluate_horizon(cur, panels, horizon_days, universe, label):
    ...
    pred_std_all = []  # §9.1 probabilistic: predicted distribution scale (audit only; top-20 用 mean)
    ...
    for i in range(1, len(panel_keys)):
        ...
        model = NGBRegressor(Dist=Normal, n_estimators=NGB_N_ESTIMATORS,
                             learning_rate=NGB_LEARNING_RATE, verbose=False, random_state=SEED)
        model.fit(X_tr_s, y_tr)
        pred_te = model.predict(X_te_s)  # MEAN of predicted Normal → top-20 ranking
        panel_pa.append((pred_te, y_test))
        try:
            pred_std_all.extend([float(s) for s in model.pred_dist(X_te_s).scale])
        except Exception:
            pass
    ...
    pred_std_stat = {"mean": float(np.mean(pred_std_all)), "median": float(np.median(pred_std_all))} if pred_std_all else None
    return result, pred_std_stat


def main():
    global SEED
    parser = argparse.ArgumentParser(description=f"Multi-Cycle NGBoost Validation {TOOL_VER}")
    ...
    SEED = args.seed  # §一.10 #3: inject run seed into NGBoost
    ...
    import ngboost
    logger.info("="*100)
    logger.info(f"Multi-Cycle NGBoost Validation {TOOL_VER}(per §14.7-CY / §14.7-DG / §9.1)")
    logger.info("="*100)
    logger.info(f"  Horizons: {horizon_labels}")
    logger.info(f"  ngboost ver: {ngboost.__version__}")
    logger.info(f"  Model:    NGBRegressor(Dist=Normal, n_estimators={NGB_N_ESTIMATORS}, lr={NGB_LEARNING_RATE})")
    ...
    try:
        ...
        for label, days in horizon_labels:
            r, std_stat = evaluate_horizon(cur, panels, days, universe, label)
            if r: results[label] = r
            if std_stat: pred_std_stats[label] = std_stat

        logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix(NGBoost)\n{'='*100}")
        ...
        if args.output or args.commit:
            output_path = args.output or f"reports/multi_cycle_ngboost_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
