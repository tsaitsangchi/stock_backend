"""
multi_cycle_stockmixer_validation.py v0.1 (StockMixer · MLP indicator×time×stock mixing · AAAI 2024 / Fan & Shen · Multi-Cycle Stock-Price Validation · per CLAUDE.md §一.11 三段式)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: StockMixer(MLP-BASED indicator×time×stock MIXING / AAAI 2024 / Fan & Shen)4-HORIZON WALK-FORWARD VALIDATION + §14.7-CY HORIZON-DOCTRINE 第二族(neural)+ §14.7-DC v0.18 SOURCE-PURE UNIVERSE + §一.10 SOURCE-TRACEABLE(全 DB)+ §一.10 #3 MULTI-RUN + 共同比較基準(COMMON COMPARISON BASELINE)新增實作(StockMixer)+ §一.11 三段式合規 + §14.7-DE §0.0-I panel-date helper
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 StockMixer(全 MLP 的指標×時間×個股「混合」模型) 序列模型,吃每支股票的「歷史價格序列」,預測未來報酬,評估「靠它選股能不能賺錢、準不準、可不可信」。
...
1. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切分析模組): 本程式為 evaluation 後處理工具,非序列落地鏈模組。
   **治權邊界**:(a) §3.2 evaluation 模組;(b) 不涉五套禁令(§0.1/§0.2/§0.3);(c) T1-T3 不分層;
   ...
   (g) 唯一職責:StockMixer 4-horizon walk-forward 預測 + 共同比較基準 metrics + JSON 持久化。
2. **[Common Comparison Baseline]** (v0.1, reports/common_model_comparison_baseline_v1.md): 本程式為共同比較基準之**新增實作(StockMixer)** —
   ...
   **完全相同** realized targets → 精準度(precision)/ 信任度(trust)比較 apples-to-apples。模型用各自 natural representation
   (tree=38 cross-sectional features;TFT=每股 weekly 序列 + GroupNormalizer pooled;StockMixer=**跨股 weekly return matrix + indicator×time×stock 三路 MLP 混合**),
...
5. **[Real StockMixer — MLP-based Mixing Architecture]** (v0.1, Fan & Shen, AAAI 2024 "StockMixer: A Simple yet Strong MLP-based
   Architecture for Stock Price Forecasting"): 全 MLP、無 attention。單一 indicator(weekly return)→ channel-lift(scalar → C channels)→ 三路混合
   (each residual + LayerNorm):(1) **time-mixing MLP** 沿 lookback L 軸(Linear(L→hidden)→GELU→Linear(hidden→L)),(2) **stock-mixing MLP**
   沿 variate N 軸(transpose → Linear(N→hidden)→GELU→Linear(hidden→N) → transpose back,paper 之核心市場/跨股 pathway),混合表徵相加後 project →
   channel collapse → temporal head Linear(L→S) → forecast 未來 S weeks。非 surrogate;從頭以 torch.nn 實作,與 multi_cycle_itransformer_validation.py 同慣例。
   variable-#variates 由 train loop 之 zero-fill + masked loss 處理(固定 N 之 stock-mixing 對 zeroed column 仍合法)。
6. **[Point-Forecast → Calibration N/A]** (v0.1): StockMixer 原始為 point forecast(MSE loss),非 quantile 模型 →
...
### Group B. StockMixer Model(跨股 weekly return matrix → indicator×time×stock 混合 → forecast)

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Return matrix | load_return_matrix() → daily close → calendar-week → log-return matrix[week × stock]| §一.10 / §一.13 source-pure |
| B.2 Model | StockMixer:channel-lift(1→C)+ time-mix MLP(L 軸)+ stock-mix MLP(N 軸)+ head(L→S)| Fan & Shen AAAI 2024 |
| B.3 Train | train_stockmixer() → Adam + MSE(masked)+ grad-clip(CPU)| MLP 三路混合 |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 JSON | --output reports/stockmixer_v0/<...>.json(schema-compatible + precision/trust)| §一.10 / §二.4 |
...
| 場景 | 命令 |
| :--- | :--- |
| Smoke(plumbing 驗證)| `python scripts/evaluation/multi_cycle_stockmixer_validation.py --smoke --output reports/stockmixer_v0/_smoke.json` |
| 完整單 seed | `... --seed 5422 --output reports/stockmixer_v0/smx_s5422.json` |
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-06-02 | Codex | **首版**:StockMixer(MLP-based indicator×time×stock mixing / AAAI 2024 / Fan & Shen)multi-cycle 股價預測驗證 + 共同比較基準新增實作。跨股 weekly return matrix → channel-lift(1→C)→ time-mixing MLP(沿 lookback L 軸)+ stock-mixing MLP(沿 variate N 軸,跨股市場 pathway)→ project + channel collapse → temporal head(L→S)→ 4-horizon cumsum scores。與 baseline 同 universe(v0.18/398)/ panels(§14.7-DE data-driven)/ forward returns(TaiwanStockPriceAdj)/ portfolio(top-20)/ cost(0.6%)/ gate(T_CZ-6)。precision(rank-IC / dir-acc / RMSE / MAE / R²)+ trust(Eff-t / 多 seed;calibration N/A 因 point-forecast)。§一.10 全 DB source-traceable;§一.10 #3 multi-seed;§8.5 leakage-safe by lookback/forecast split;§一.11 三段式合規。 | **ACTIVE** |
"""
...
# ════════════════════════════════════════════════════════════════════════════
# Group B — StockMixer return-matrix construction (variates = stocks)
# ════════════════════════════════════════════════════════════════════════════
...
# ── StockMixer architecture (from-scratch torch.nn; faithful MLP-based mixing) ──
def _build_model(N, L, S, d_model, channels, dropout):
    import torch.nn as nn

    class StockMixer(nn.Module):
        def __init__(self):
            super().__init__()
            self.lift = nn.Linear(1, channels)               # single indicator scalar → C channels
            self.time_mix = nn.Sequential(
                nn.Linear(L, d_model), nn.GELU(), nn.Dropout(dropout), nn.Linear(d_model, L),
            )                                                # MLP across the L (time) axis
            self.time_norm = nn.LayerNorm(L)
            self.stock_mix = nn.Sequential(
                nn.Linear(N, d_model), nn.GELU(), nn.Dropout(dropout), nn.Linear(d_model, N),
            )                                                # MLP across the N (stock) axis — market pathway
            self.stock_norm = nn.LayerNorm(N)
            self.proj = nn.Linear(channels, channels)        # combine mixed channel representations
            self.head = nn.Linear(L, S)                      # collapsed channels → future S weekly returns

        def forward(self, x, key_padding_mask=None):         # x:[B,N,L]  mask:[B,N] True=ignore (may be ignored)
            h = self.lift(x.unsqueeze(-1))                   # [B,N,L,C]
            h = h.permute(0, 1, 3, 2)                        # [B,N,C,L]
            h = self.time_norm(h + self.time_mix(h))         # time-mix + residual + LayerNorm → [B,N,C,L]
            sm_in = h.permute(0, 2, 3, 1)                    # [B,C,L,N]
            sm = self.stock_norm(sm_in + self.stock_mix(sm_in))  # stock-mix + residual + LayerNorm → [B,C,L,N]
            h = sm.permute(0, 2, 3, 1)                        # [B,L,N,C]
            h = self.proj(h).mean(dim=-1)                    # combine channels → collapse → [B,L,N]
            h = h.permute(0, 2, 1)                           # [B,N,L]
            return self.head(h)                              # [B,N,S]

    return StockMixer()


def train_stockmixer(ret_mat, week_cut, L, S, seed, epochs, lr, d_model, channels,
                     batch_size, dropout, min_valid=N_TOP + 5):
    """Train StockMixer on all windows whose lookback+target ∈ weeks ≤ week_cut (leakage-safe).
    ...
    model = _build_model(ret_mat.shape[1], L, S, d_model, channels, dropout)  # N = #stocks (fixed)
    ...
    for _ep in range(epochs):
        ...
        for bs in range(0, len(usable), batch_size):
            ...
            kpm = ~lbv                                             # True = padded variate (zeroed column)
...
def aggregate_horizon(label, horizon_days, panel_top_rets, panel_univ_rets,
    ...
    return {
        ...
        "calibration_p10_p90_coverage": calib_cov,  # None for StockMixer (point forecast, non-quantile)
    }
...
def run(args):
    ...
    logger.info(f"StockMixer refit at panel indices {refit_points} (every {refit_every}); "
                f"L={L}wk S={S}wk d_model={args.d_model} channels={args.channels} "
    ...
    for pi, as_of in enumerate(panels):
        ...
        if pi in refit_points or model is None:
            tr0 = time.monotonic()
            res = train_stockmixer(ret_mat, wc, L, S, args.seed, args.epochs, args.lr,
                                   args.d_model, args.channels, args.batch_size, args.dropout)
            if res is None:
...
def main():
    ap = argparse.ArgumentParser(description=f"Multi-Cycle StockMixer Validation {TOOL_VER}")
    ...
    ap.add_argument("--d-model", type=int, default=64, help="hidden width of mixing MLPs")
    ap.add_argument("--channels", type=int, default=4, help="indicator channel-lift dimension")
    ...
    logger.info("=" * 100)
    logger.info(f"Multi-Cycle StockMixer Validation {TOOL_VER} (MLP indicator×time×stock mixing / AAAI 2024)")
    ...
    logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix (StockMixer)\n{'='*100}")
    ...
    if args.output:
        ...
        out["_meta"] = {
            "tool": "multi_cycle_stockmixer_validation.py", "tool_ver": TOOL_VER,
            "model": "StockMixer (MLP indicator×time×stock mixing, AAAI 2024, from-scratch torch.nn)",
            ...
            "d_model": args.d_model, "channels": args.channels,
            "refit_every": args.refit_every, "epochs": args.epochs, "smoke": args.smoke,
            "variates": "stocks (cross-sectional, MLP stock-mixing pathway)",
