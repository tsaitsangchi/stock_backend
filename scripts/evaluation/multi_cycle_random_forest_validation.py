"""
multi_cycle_random_forest_validation.py v0.2 (Multi-Cycle Random Forest Validation + Precision/Reliability Analysis · §14.7-CY 第五實作 · per Canonical Comparison Framework · per CLAUDE.md §一.11 三段式入憲)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: MULTI-CYCLE 4-HORIZON RANDOM FOREST VALIDATION + PRECISION/RELIABILITY + CANONICAL COMPARISON FRAMEWORK + §14.7-CY HORIZON-EXTENSION + §14.7-CX 8-YEAR OOS + §14.7-CW TREE-FAMILY 第五實作 + §一.10 SOURCE-TRACEABLE + §一.11 三段式合規 + §14.7-DE/DF §0.0-I 雙 helper 切換(2026-06-02)
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 Random Forest(隨機森林) 模型,實測「靠過去的股票特徵能不能預測未來、選股賺錢」,並給出「賺多少 / 準不準 / 可不可信」的成績單。
...
2. **[Random Forest Bagging Ensemble]** (v0.1): 200 trees / max_depth=5 / bootstrap=True / max_features='sqrt' / seed=5422。
3. **[Canonical Comparison Framework]** (v0.1, 新建): metrics 與 LGBM/XGBoost/CatBoost/Ensemble validators 完全 standardized。
4. **[Overlap-Corrected n_effective]** (v0.1, §14.7-CY T_CY-3): n_eff = n × (30/horizon)。
5. **[Honest Annualization]** (v0.1, §14.7-CY T_CY-4): mean × (252/horizon)。
...
10. **[Source Traceability]** (v0.1, CLAUDE.md §一.10): 全 (b) DB query + (a) program output;0 AI memory。
11. **[Zero Hardcoded Verdict]** (v0.1, §5.6.3): significance + precision tier 動態判定。
12. **[Sovereignty Declaration]** (v0.1, §3.1 序列模組): 本程式為 **§14.7-CY 第五 evaluation 實作**(LGBM/XGB/CatBoost/Ensemble 為前四)。**治權邊界**:(a) §3.1 evaluation;(b) read-only;(c) 不訓練 production model;(d) 不修改 DB;(e) 唯一職責:RF 4-horizon walk-forward + precision/reliability + JSON 持久化。
13. **[Historical Reference Authority]** (v0.1): RF 為 bagging baseline;與 GBT family 對比。
...
### Group B. Walk-Forward RF Training (per horizon)
| 子項 | 對應方法 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Expanding window | train [0..i-1] → test i | §14.7-CW T_CW-2 |
| B.2 RF params | RF_PARAMS(200/5/sqrt/5422)| §14.7-CW T_CW-4 |
| B.3 Bagging | bootstrap=True | RF essence |
...
| F.2 JSON output | `reports/multi_cycle_random_forest_<ts>.json` | §一.10 |

### 對齊憲章 §二 維運矩陣
| 場景 | 命令 |
| :--- | :--- |
| 日常 dry-run | `python scripts/evaluation/multi_cycle_random_forest_validation.py --dry-run` |
...
| v0.1 | 2026-05-29 | Codex | **首版:§14.7-CY 第五實作(Random Forest)**。4-horizon walk-forward + precision/reliability。Canonical Comparison Framework 對齊。 | SUPERSEDED |
"""
...
RF_PARAMS = {"n_estimators": 200, "max_depth": 5, "min_samples_leaf": 30, "max_features": "sqrt", "bootstrap": True, "random_state": SEED, "n_jobs": -1}
...
def main():
    parser = argparse.ArgumentParser(description=f"Multi-Cycle Random Forest Validation {TOOL_VER}")
    ...
    RF_PARAMS["random_state"] = args.seed; globals()["SEED"] = args.seed  # §一.10 #3: inject run seed
    ...
    logger.info("="*100)
    logger.info(f"Multi-Cycle Random Forest Validation {TOOL_VER}(per §14.7-CY)")
    logger.info("="*100)
    ...
    try:
        ...
        logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix(Random Forest)+ Precision/Reliability\n{'='*100}")
        ...
        if args.output or args.commit:
            output_path = args.output or f"reports/multi_cycle_random_forest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            output_results["_meta"] = {
                "tool": "multi_cycle_random_forest_validation.py", "tool_ver": TOOL_VER,
                "model_family": "random_forest", "sklearn_version": sklearn.__version__,
