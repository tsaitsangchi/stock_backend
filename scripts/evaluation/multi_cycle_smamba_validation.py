"""
multi_cycle_smamba_validation.py v0.1 (S-Mamba · Selective State-Space / Mamba · 2024 / Wang et al. + Gu & Dao 2023 · Multi-Cycle Stock-Price Validation · per CLAUDE.md §一.11 三段式)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: S-MAMBA(SELECTIVE STATE-SPACE / MAMBA, 2024)4-HORIZON WALK-FORWARD VALIDATION + §14.7-CY HORIZON-DOCTRINE 神經族 + §14.7-DC v0.18 SOURCE-PURE UNIVERSE + §一.10 SOURCE-TRACEABLE(全 DB)+ §一.10 #3 MULTI-RUN + 共同比較基準(COMMON COMPARISON BASELINE)新增實作 + §一.11 三段式合規 + §14.7-DE §0.0-I panel-date helper + PURE-TORCH SSM(無 CUDA / 無 mamba-ssm)

**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 S-Mamba(選擇性狀態空間 / Mamba 序列模型,純 PyTorch 自寫、無 CUDA) 序列模型,吃每支股票的「歷史價格序列」,預測未來報酬,評估「靠它選股能不能賺錢、準不準、可不可信」。
...
**它不做的事**:不改資料庫(純讀取評估;§3.1 evaluation 角色);不依賴 CUDA 或 mamba-ssm 套件(選擇性掃描以純 torch for-loop 實作,CPU 正確但較慢)。
...
1. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切分析模組): 本程式為 evaluation 後處理工具,非序列落地鏈模組。
   **治權邊界**:(a) §3.2 evaluation 模組;(b) 不涉五套禁令(§0.1/§0.2/§0.3);(c) T1-T3 不分層;
   ...
   (g) 唯一職責:S-Mamba 4-horizon walk-forward 預測 + 共同比較基準 metrics + JSON 持久化。
2. **[Common Comparison Baseline]** (v0.1, reports/common_model_comparison_baseline_v1.md): 本程式為共同比較基準之**新增實作**(S-Mamba)—
   ...
   **完全相同** realized targets → 精準度(precision)/ 信任度(trust)比較 apples-to-apples。模型用各自 natural representation
   (tree=38 cross-sectional features;TFT=每股 weekly 序列 + GroupNormalizer pooled;S-Mamba=**每股 weekly return 序列之選擇性狀態空間掃描**),
...
5. **[Real S-Mamba — Selective State-Space, PURE TORCH]** (v0.1, Gu & Dao 2023 "Mamba: Linear-Time Sequence Modeling with Selective State Spaces"
   + Wang/Wu/Shu/Chen/et al. 2024 "Is Mamba Effective for Time Series Forecasting? (S-Mamba)"): 核心 = **channel-independent** 對每支股票之 lookback
   return 序列(reshape [B*N, L, 1])獨立過 Mamba 選擇性掃描;MambaBlock 含 input projection + SiLU gate expand branch + 選擇性 SSM
   (input-dependent Δ=softplus(Linear) / B=Linear / C=Linear 逐時步;A = -exp(A_log) diagonal 學習 [d_state];離散化 Ā=exp(Δ·A), B̄=Δ·B;
   **SEQUENTIAL SCAN** 以 python/torch for-loop 逐 L 時步更新 state h=Ā*h+B̄*x, y=C*h)。
   ⚠️ **PURE PyTorch from-scratch**:本機為 Intel-Mac CPU 無 CUDA → **不 import mamba-ssm / causal-conv1d / 任何 custom CUDA kernel**;掃描為 plain torch for-loop(慢但 CPU-correct);
   d_state 保持 SMALL(default 4)、epochs/size modest 以控制 sequential-scan 成本。最終 last-step → project(d_model→S)產 [B*N,S] → reshape [B,N,S]。
   key_padding_mask 被忽略(padding 變量已於 loss 端 mask)。
...
6. **[Point-Forecast → Calibration N/A]** (v0.1): S-Mamba 此處為 point forecast(MSE loss),非 quantile 模型 →
...
| A.2 Panels | get_canonical_panel_dates() → data-driven monthly mid-month| 與 baseline 同 grid / §14.7-DE |
...
### Group B. S-Mamba Model(每股 weekly return 序列 → 選擇性狀態空間掃描 → forecast)

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Return matrix | load_return_matrix() → daily close → calendar-week → log-return matrix[week × stock]| §一.10 / §一.13 source-pure |
| B.2 Model | SMamba:channel-indep MambaBlock(SiLU-gate + 選擇性 SSM sequential scan)+ project(d→S)| Gu & Dao 2023 / Wang et al. 2024(pure-torch)|
| B.3 Train | train_smamba() → Adam + MSE(masked)+ grad-clip(CPU)| 純 torch SSM |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 JSON | --output reports/smamba_v0/<...>.json(schema-compatible + precision/trust)| §一.10 / §二.4 |
...
| 場景 | 命令 |
| :--- | :--- |
| Smoke(plumbing 驗證)| `python scripts/evaluation/multi_cycle_smamba_validation.py --smoke --output reports/smamba_v0/_smoke.json` |
| 完整單 seed | `... --seed 5422 --output reports/smamba_v0/smb_s5422.json` |
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-06-02 | Codex | **首版**:S-Mamba pure-torch(Selective State-Space / Mamba 2024 / Gu & Dao 2023 + Wang et al. S-Mamba)multi-cycle 股價預測驗證 + 共同比較基準新增實作。每股 weekly return 序列 channel-independent → MambaBlock(input-proj + SiLU-gate expand + 選擇性 SSM:input-dependent Δ/B/C + diagonal A=-exp(A_log) + 離散化 Ā/B̄ + SEQUENTIAL SCAN 純 torch for-loop)→ last-step project 未來 S weeks → 4-horizon cumsum scores。⚠️ Intel-Mac CPU 無 CUDA → 純 PyTorch from-scratch,不 import mamba-ssm/causal-conv1d,d_state 小(8)、size modest。與 baseline 同 universe(v0.18/398)/ panels(data-driven §14.7-DE)/ forward returns(TaiwanStockPriceAdj)/ portfolio(top-20)/ cost(0.6%)/ gate(T_CZ-6)。precision(rank-IC / dir-acc / RMSE / MAE / R²)+ trust(Eff-t / 多 seed;calibration N/A 因 point-forecast)。§一.10 全 DB source-traceable;§一.10 #3 multi-seed;§8.5 leakage-safe by lookback/forecast split;§一.11 三段式合規。 | ACTIVE |
"""
...
# ════════════════════════════════════════════════════════════════════════════
# Group B — S-Mamba return-matrix construction (per-stock weekly return series)
# ════════════════════════════════════════════════════════════════════════════
...
# ── S-Mamba architecture (from-scratch torch.nn; pure-torch selective SSM, no CUDA) ──
def _build_model(L, S, d_model, d_state, n_blocks, dropout):
    ...
    class MambaBlock(nn.Module):
        """Pure-torch selective state-space block (Gu & Dao 2023, diagonal SSM, sequential scan)."""
        def __init__(self, d_model, d_state, dropout):
            super().__init__()
            self.d_model = d_model
            self.d_state = d_state
            self.in_proj = nn.Linear(d_model, 2 * d_model)        # main branch + SiLU gate branch
            self.x_proj = nn.Linear(d_model, d_state * 2)         # input-dependent B, C per time step
            self.dt_proj = nn.Linear(d_model, d_model)            # input-dependent Δ (softplus → per-channel)
            self.A_log = nn.Parameter(torch.log(torch.arange(1, d_state + 1, dtype=torch.float32)
                                                .unsqueeze(0).repeat(d_model, 1)))  # [d_model,d_state] diagonal A
            self.D = nn.Parameter(torch.ones(d_model))            # skip connection
            self.out_proj = nn.Linear(d_model, d_model)
            self.norm = nn.LayerNorm(d_model)
            self.drop = nn.Dropout(dropout)

        def forward(self, u):                                     # u:[B,Lseq,d_model]
            residual = u
            u = self.norm(u)
            xz = self.in_proj(u)                                  # [B,Lseq,2*d_model]
            x, z = xz.chunk(2, dim=-1)                            # main / gate
            x = F.silu(x)                                         # activate main branch
            B_, Lseq, _ = x.shape
            A = -torch.exp(self.A_log)                            # [d_model,d_state] negative real diagonal
            bc = self.x_proj(x)                                   # [B,Lseq,2*d_state]
            Bm, Cm = bc.split(self.d_state, dim=-1)               # each [B,Lseq,d_state] input-dependent
            dt = F.softplus(self.dt_proj(x))                      # [B,Lseq,d_model] Δ per channel
            h = torch.zeros(B_, self.d_model, self.d_state, device=x.device, dtype=x.dtype)  # state
            ys = []
            for t in range(Lseq):                                # SEQUENTIAL SCAN (pure torch for-loop, CPU)
                dt_t = dt[:, t]                                   # [B,d_model]
                # discretize: Ā=exp(Δ·A), B̄=Δ·B  (broadcast over d_state)
                dA = torch.exp(dt_t.unsqueeze(-1) * A.unsqueeze(0))           # [B,d_model,d_state]
                dB = dt_t.unsqueeze(-1) * Bm[:, t].unsqueeze(1)               # [B,d_model,d_state]
                h = dA * h + dB * x[:, t].unsqueeze(-1)                       # h=Ā*h+B̄*x  [B,d_model,d_state]
                y_t = (h * Cm[:, t].unsqueeze(1)).sum(dim=-1)                 # y=C*h  [B,d_model]
                ys.append(y_t)
            y = torch.stack(ys, dim=1)                            # [B,Lseq,d_model]
            y = y + x * self.D.view(1, 1, -1)                     # skip
            y = y * F.silu(z)                                     # gate output by SiLU branch
            y = self.out_proj(y)
            return residual + self.drop(y)                        # residual (norm applied at entry)

    class SMamba(nn.Module):
        def __init__(self):
            super().__init__()
            self.embed = nn.Linear(1, d_model)                   # channel-indep: per-step scalar return → d_model
            self.blocks = nn.ModuleList([MambaBlock(d_model, d_state, dropout) for _ in range(n_blocks)])
            self.norm = nn.LayerNorm(d_model)
        ...
        def forward(self, x, key_padding_mask=None):             # x:[B,N,L]  mask ignored (loss-masked upstream)
            B_, N, Lseq = x.shape
            seq = x.reshape(B_ * N, Lseq, 1)                     # channel-independent over stocks
            h = self.embed(seq)                                  # [B*N,L,d_model]
            for blk in self.blocks:
                h = blk(h)
            h = self.norm(h)
            out = self.head(h[:, -1])                            # last-step → [B*N,S]
            return out.reshape(B_, N, S)                         # [B,N,S]

    return SMamba()


def train_smamba(ret_mat, week_cut, L, S, seed, epochs, lr, d_model, d_state, n_blocks,
                 batch_size, dropout, max_train_windows=None, min_valid=N_TOP + 5):
    """Train S-Mamba on all windows whose lookback+target ∈ weeks ≤ week_cut (leakage-safe).
...
def aggregate_horizon(label, horizon_days, panel_top_rets, panel_univ_rets,
    ...
    return {
        ...
        "calibration_p10_p90_coverage": calib_cov,  # None for S-Mamba (point forecast, non-quantile)
    }
...
def run(args):
    ...
    logger.info(f"S-Mamba refit at panel indices {refit_points} (every {refit_every}); "
                f"L={L}wk S={S}wk d_model={args.d_model} d_state={args.d_state} blocks={args.n_blocks} "
    ...
    for pi, as_of in enumerate(panels):
        ...
        if pi in refit_points or model is None:
            tr0 = time.monotonic()
            res = train_smamba(ret_mat, wc, L, S, args.seed, args.epochs, args.lr,
...
def main():
    ap = argparse.ArgumentParser(description=f"Multi-Cycle S-Mamba Validation {TOOL_VER}")
    ...
    ap.add_argument("--d-state", type=int, default=4, help="SSM state size (keep small — sequential scan is slow on CPU)")
    ...
    logger.info("=" * 100)
    logger.info(f"Multi-Cycle S-Mamba Validation {TOOL_VER} (Selective State-Space / Mamba 2024, pure-torch CPU)")
    ...
    logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix (S-Mamba)\n{'='*100}")
    ...
    if args.output:
        ...
        out["_meta"] = {
            "tool": "multi_cycle_smamba_validation.py", "tool_ver": TOOL_VER,
            "model": "S-Mamba (Selective State-Space / Mamba, 2024, pure-torch from-scratch, CPU)",
            ...
            "variates": "stocks (channel-independent per-stock weekly return sequence)",
