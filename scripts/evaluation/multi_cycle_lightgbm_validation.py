"""
multi_cycle_lightgbm_validation.py v0.2 (Multi-Cycle LightGBM Validation + Precision/Reliability Analysis · §14.7-CY 第七實作 dedicated · per Canonical Comparison Framework · per CLAUDE.md §一.11 三段式入憲)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: MULTI-CYCLE 4-HORIZON LIGHTGBM VALIDATION + PRECISION/RELIABILITY + CANONICAL COMPARISON FRAMEWORK + §14.7-CY HORIZON-EXTENSION + §14.7-CX 8-YEAR OOS + §14.7-CW TREE-FAMILY 第七實作 dedicated + §一.10 SOURCE-TRACEABLE + §一.11 三段式合規 + §14.7-DE/DF §0.0-I 雙 helper 切換(2026-06-02)
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 LightGBM 機器學習模型,實測「靠過去的股票特徵能不能預測未來、選股賺錢」,並給出「賺多少 / 準不準 / 可不可信」的成績單。

**它怎麼做(步驟)**:
1. 取 397 支「乾淨核心股」+ 每股 37 個經驗證的特徵(估值、動能、籌碼、財務等;全來自真實 FinMind/FRED API,無 AI 亂補值)。
2. 把 2013-05 ~ 2026-06 切成 157 個月度時間點(panel)。
3. **逐點往前走(walk-forward)**:每個時間點「只用那之前的資料」訓練模型,預測「之後」每支股票的報酬 —— 模擬當時真的只看得到過去、不偷看未來(防資料洩漏)。
4. 依預測分數挑最看好的 **前 20 支等權做多**,跟「全市場平均」比,算這一期賺賠。
5. 上面在 **4 種持有期**各做一遍:週(5 天)/ 月(20 天)/ 季(60 天)/ 年(252 天)。
6. 算出每種週期的成績:**報酬率、Sharpe(風險調整後績效)、勝率、預測命中率、跨次穩定度(可信度)、扣交易成本後年化報酬**。
7. 用門檻(§14.7-CZ T_CZ-6:有效 t 值 ≥ 4.20 且 Sharpe ≥ 2.40 且 勝率 ≥ 79%)判定:**這模型在哪個週期「真的能賺錢且可信」**。

**輸入**:資料庫(feature_values 特徵 + TaiwanStockPriceAdj 股價)。**輸出**:一份 JSON(各週期成績 + 來源標記)+ 終端機 log。
**它不做的事**:不訓練正式上線模型、不改資料庫(純讀取做評估;§3.1 evaluation 角色)。
**為什麼需要它**:這是回答「股票預測到底能不能賺錢」的實證裁判;且 XGBoost / CatBoost 等所有模型都用「同一支標準、同一窗、同一指標」跑 → 成績才能公平並排比較。
...
2. **[LightGBM Leaf-Wise GBT]** (v0.1): 200 trees / learning_rate=0.05 / max_depth=5 / num_leaves=20 / min_child_samples=30 / feature_fraction=0.8 / bagging_fraction=0.8 / reg_alpha=0.1 / reg_lambda=0.1 / seed=5422;LightGBM 之 leaf-wise(best-first)growth 在 small datasets 須 max_depth + num_leaves 雙保險。
3. **[Canonical Comparison Framework]** (v0.1, per RF 建立): metrics 與 LGBM v0.2 / XGBoost / CatBoost / Ensemble / Random Forest / Extra Trees validators 完全 standardized,確保 7-tree comparison reliable。
...
12. **[Sovereignty Declaration]** (v0.1, §3.1 序列模組): 本程式為 **§14.7-CY 第七 evaluation 實作 dedicated**(LGBM v0.2 / XGB / CatBoost / Ensemble / Random Forest / Extra Trees 為前六)。**治權邊界**:(a) §3.1 evaluation;(b) read-only;(c) 不訓練 production model;(d) 不修改 DB;(e) 唯一職責:LightGBM 4-horizon walk-forward + precision/reliability + JSON 持久化。
13. **[Historical Reference Authority]** (v0.1): LGBM v0.2 production multi-cycle 結果為 §14.7-CY/CZ 治權基準(quarterly Eff t=4.20 / Sharpe 2.55 / NetAnn +24.44%);本 dedicated v0.1 為 Canonical Comparison Framework 對齊版本,reproducibility-aware。
...
### Group B. Walk-Forward LightGBM Training (per horizon)
| 子項 | 對應方法 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Expanding window | train [0..i-1] → test i | §14.7-CW T_CW-2 |
| B.2 LGBM params | LGBM_PARAMS(200/0.05/5/20/30/0.8/0.8/0.1/0.1/5422)| §14.7-CW T_CW-4 |
| B.3 Leaf-wise growth | `boosting_type='gbdt'` | LGBM essence |
...
| F.2 JSON output | `reports/multi_cycle_lightgbm_<ts>.json` | §一.10 |

### 對齊憲章 §二 維運矩陣
| 場景 | 命令 |
| :--- | :--- |
| 日常 dry-run | `python scripts/evaluation/multi_cycle_lightgbm_validation.py --dry-run` |
...
| v0.1 | 2026-05-29 | Codex | **首版:§14.7-CY 第七實作 dedicated(LightGBM)** under Canonical Comparison Framework。 (1) 4-horizon walk-forward,LightGBM LGBMRegressor;(2) 與 multi_cycle_validation.py(LGBM v0.2 production §14.7-CY 基準)並存;此 dedicated 版本為 Canonical Comparison Framework 對齊;(3) Precision/Reliability 新層延續其他 multi-cycle validators;(4) Hyperparameters 對齊 LGBM v0.2 production(200/0.05/5/20/30/0.8/0.8/0.1/0.1/5422)。 | SUPERSEDED |
"""
...
def evaluate_horizon(cur, panels, horizon_days, universe, label):
    ...
    panel_pa = []  # §14.7-DF: 每 OOS panel 之 (pred, actual) → 共用 helper 算 metric(單一來源)
    ...
    logger.info(f"    OOS panels: {result['n_panels']} | n_eff: {result['n_effective']:.1f}")
    logger.info(f"    Sharpe: {result['sharpe']:+.4f} | Win: {result['win_rate']*100:.1f}% | α: {result['mean_alpha_per_panel']*100:+.4f}% | IR: {result['ir']:+.4f}")
    logger.info(f"    Eff t-stat: {result['effective_t_stat']:+.3f} | Sig p<0.05: {'✅' if result['is_significant_p05'] else '❌'}")
    logger.info(f"    Annualized NET: {result['annualized_simple_net']*100:+.2f}%/yr | Mean IC: {result['mean_ic']:+.4f}")
    logger.info(f"    --- Precision --- hit {result['precision_directional_hit_rate']*100:.1f}% | top-20 overlap {result['precision_top20_actual_overlap']*100:.1f}% | RMSE {result['precision_rmse']:.4f} | MAE {result['precision_mae']:.4f}")
    logger.info(f"    --- Reliability --- IC CoV {result['reliability_ic_stability_cov']:.4f}")
    return result


def main():
    parser = argparse.ArgumentParser(description=f"Multi-Cycle LightGBM Validation {TOOL_VER}(Canonical Comparison Framework)")
    ...
    LGBM_PARAMS["seed"] = args.seed; globals()["SEED"] = args.seed  # §一.10 #3: inject run seed
    ...
    logger.info("="*100)
    logger.info(f"Multi-Cycle LightGBM Validation {TOOL_VER}(per §14.7-CY / Canonical Comparison Framework)")
    logger.info("="*100)
    ...
    try:
        ...
        panels = get_canonical_panel_dates("feature_set_v0.5")  # §14.7-DE / §0.0-I 單一引用源(資料驅動、反硬編)
        logger.info(f"  Panels: {len(panels)}(2013-06-15 ~ 2026-04-15 monthly)")
        ...
        logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix(LightGBM dedicated)+ Precision/Reliability\n{'='*100}")
        ...
        if args.output or args.commit:
            output_path = args.output or f"reports/multi_cycle_lightgbm_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            output_results["_meta"] = {
                "tool": "multi_cycle_lightgbm_validation.py", "tool_ver": TOOL_VER,
                "model_family": "lightgbm", "lightgbm_version": lgb.__version__,
