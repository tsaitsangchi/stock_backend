"""
multi_cycle_master_validation.py v0.1 (MASTER · Market-Guided Stock Transformer · AAAI 2024 / Li et al. · Multi-Cycle Stock-Price Validation · per CLAUDE.md §一.11 三段式)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: MASTER(MARKET-GUIDED STOCK TRANSFORMER / AAAI 2024)4-HORIZON WALK-FORWARD VALIDATION + §14.7-CY HORIZON-DOCTRINE 神經族(neural)+ §14.7-DC v0.18 SOURCE-PURE UNIVERSE + §一.10 SOURCE-TRACEABLE(全 DB)+ §一.10 #3 MULTI-RUN + 共同比較基準(COMMON COMPARISON BASELINE)新增實作 + §一.11 三段式合規 + §14.7-DE §0.0-I panel-date helper
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 MASTER(市場引導的個股 Transformer) 序列模型,吃每支股票的「歷史價格序列」,並用「整體市場狀態」去引導對每支股票的判斷,預測未來報酬,評估「靠它選股能不能賺錢、準不準、可不可信」。
...
1. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切分析模組): 本程式為 evaluation 後處理工具,非序列落地鏈模組。
   **治權邊界**:(a) §3.2 evaluation 模組;(b) 不涉五套禁令(§0.1/§0.2/§0.3);(c) T1-T3 不分層;
   ...
   (g) 唯一職責:MASTER 4-horizon walk-forward 預測 + 共同比較基準 metrics + JSON 持久化。
2. **[Common Comparison Baseline]** (v0.1, reports/common_model_comparison_baseline_v1.md): 本程式為共同比較基準之**新增實作 (MASTER)** —
   ...
   **完全相同** realized targets → 精準度(precision)/ 信任度(trust)比較 apples-to-apples。模型用各自 natural representation
   (tree=38 cross-sectional features;TFT=每股 weekly 序列 + GroupNormalizer pooled;MASTER=**跨股多變量 weekly return matrix + 市場引導跨股 attention**),
...
5. **[Real MASTER — Market-Guided Stock Transformer]** (v0.1, Li/Wang/Mei/Lin/Yang/An, AAAI 2024 "MASTER: Market-Guided Stock
   Transformer for Stock Price Forecasting"): 核心「市場引導」= (1) 每股 lookback return 序列 embed 為 stock token h:[B,N,d];
   (2) 對 VALID stocks(~mask)沿 N 軸 mean-pool 得 market vector m:[B,d](市場表徵);(3) FiLM-style 市場閘控 gate=sigmoid(Linear(m))
   廣播調制 stock token(market guidance);(4) **跨股 self-attention**(nn.TransformerEncoder over N stock tokens,
   src_key_padding_mask 忽略 padded — paper 之核心 cross-stock 機制);(5) head project 至未來 S weeks → [B,N,S]。
   非 surrogate;從頭以 torch.nn 實作(embed Linear + market mean-pool + FiLM gate + nn.TransformerEncoder + project Linear),
   與 multi_cycle_transformer_dedicated_validation.py / multi_cycle_itransformer_validation.py 同慣例。
   variable-#variates(股數)由 key_padding_mask 處理(market pool + cross-stock attention 皆 mask-aware,天然容許不定 stock 數)。
6. **[Point-Forecast → Calibration N/A]** (v0.1): MASTER 原始為 point forecast(MSE loss),非 quantile 模型 →
...
### Group B. MASTER Model(市場引導跨股 attention → forecast)

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Return matrix | load_return_matrix() → daily close → calendar-week → log-return matrix[week × stock]| §一.10 / §一.13 source-pure |
| B.2 Model | MASTER:embed(L→d) + market mean-pool + FiLM gate + TransformerEncoder(跨股 attention)+ project(d→S)| Li et al. AAAI 2024 |
| B.3 Train | train_master() → Adam + MSE(masked)+ grad-clip(CPU)| 市場引導 multivariate |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 JSON | --output reports/master_v0/<...>.json(schema-compatible + precision/trust)| §一.10 / §二.4 |
...
| 場景 | 命令 |
| :--- | :--- |
| Smoke(plumbing 驗證)| `python scripts/evaluation/multi_cycle_master_validation.py --smoke --output reports/master_v0/_smoke.json` |
| 完整單 seed | `... --seed 5422 --output reports/master_v0/master_s5422.json` |
...
# ── MASTER / weekly constants ──
LOOKBACK_WEEKS = 104       # stock-token lookback (~2 yr)
...
# ════════════════════════════════════════════════════════════════════════════
# Group B — MASTER return-matrix construction (variates = stocks)
# ════════════════════════════════════════════════════════════════════════════
...
# ── MASTER architecture (from-scratch torch.nn; faithful market-guided cross-stock) ──
def _build_model(L, S, d_model, n_heads, n_layers, dropout):
    ...
    class MASTER(nn.Module):
        def __init__(self):
            super().__init__()
            self.embed = nn.Linear(L, d_model)               # per-stock temporal embedding: L-series → token
            self.market_gate = nn.Linear(d_model, d_model)   # FiLM gate from market vector
            self.market_proj = nn.Linear(d_model, d_model)   # market guidance bias
            layer = nn.TransformerEncoderLayer(
                d_model, n_heads, dim_feedforward=4 * d_model,
                dropout=dropout, batch_first=True, activation="gelu",
            )
            self.encoder = nn.TransformerEncoder(layer, n_layers)  # cross-stock attention
        ...
        def forward(self, x, key_padding_mask=None):         # x:[B,N,L]  mask:[B,N] True=ignore
            h = self.embed(x)                                # [B,N,d_model] per-stock tokens
            if key_padding_mask is not None:
                valid = (~key_padding_mask).float().unsqueeze(-1)        # [B,N,1] 1=valid stock
                m = (h * valid).sum(dim=1) / valid.sum(dim=1).clamp(min=1.0)  # market vector m:[B,d] over valid stocks
            else:
                m = h.mean(dim=1)                            # market vector m:[B,d]
            gate = torch.sigmoid(self.market_gate(m)).unsqueeze(1)       # [B,1,d] broadcast over N
            bias = self.market_proj(m).unsqueeze(1)                      # [B,1,d] market guidance
            h = h * gate + bias                              # FiLM-style market-guided modulation
            h = self.encoder(h, src_key_padding_mask=key_padding_mask)  # attention across stocks
            h = self.norm(h)
...
def train_master(ret_mat, week_cut, L, S, seed, epochs, lr, d_model, n_heads, n_layers,
                 batch_size, dropout, min_valid=N_TOP + 5):
    """Train MASTER on all windows whose lookback+target ∈ weeks ≤ week_cut (leakage-safe).
...
def aggregate_horizon(label, horizon_days, panel_top_rets, panel_univ_rets,
    ...
    return {
        ...
        "calibration_p10_p90_coverage": calib_cov,  # None for MASTER (point forecast, non-quantile)
    }
...
def run(args):
    ...
    logger.info(f"MASTER refit at panel indices {refit_points} (every {refit_every}); "
    ...
    for pi, as_of in enumerate(panels):
        ...
        if pi in refit_points or model is None:
            tr0 = time.monotonic()
            res = train_master(ret_mat, wc, L, S, args.seed, args.epochs, args.lr,
...
def main():
    ap = argparse.ArgumentParser(description=f"Multi-Cycle MASTER Validation {TOOL_VER}")
    ...
    logger.info("=" * 100)
    logger.info(f"Multi-Cycle MASTER Validation {TOOL_VER} (Market-Guided Stock Transformer / AAAI 2024)")
    ...
    logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix (MASTER)\n{'='*100}")
    ...
    if args.output:
        ...
        out["_meta"] = {
            "tool": "multi_cycle_master_validation.py", "tool_ver": TOOL_VER,
            "model": "MASTER (Market-Guided Stock Transformer, AAAI 2024, from-scratch torch.nn)",
            ...
            "variates": "stocks (cross-sectional multivariate, market-guided)",
