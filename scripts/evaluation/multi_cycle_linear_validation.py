"""
multi_cycle_linear_validation.py v0.1 (Multi-Cycle Linear Baseline Validation Engine · per §14.7-CY Tree Family Comparison Baseline · per CLAUDE.md §一.11 三段式入憲)
================================================================================
**最後更新日期**: 2026-06-04
**主權狀態**: MULTI-CYCLE 4-HORIZON LINEAR BASELINE VALIDATION + §14.7-CY HORIZON-EXTENSION + §14.7-CX 8-YEAR OOS EXTENSION + §一.10 SOURCE-TRACEABLE (per CLAUDE.md §一.11 強制三段式標頭;§14.7-CY T_CY-1 system script 強制;§一.10 三類唯一 source 強制) + §14.7-DE/DF §0.0-I 雙 helper(get_canonical_panel_dates / summarize_horizon_metrics)
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用「線性回歸」(OLS / Ridge / Lasso / ElasticNet) 在 37 個股票特徵上預測個股未來報酬、選最看好的前 20 支做多,作為「複雜模型(樹 / Transformer)到底有沒有贏過最簡單的線性模型」之共同比較基準。

**它怎麼做(步驟)**:
1. 取 397 支「乾淨核心股」+ 每股 37 個經驗證的特徵(估值、動能、籌碼、財務等;全來自真實 FinMind/FRED API,無 AI 亂補值)。
2. 把 2013-05 ~ 2026-06 切成多個月度時間點(panel)。
3. **逐點往前走(walk-forward)**:每個時間點「只用那之前的資料」先用 StandardScaler 標準化特徵,再 fit 四種線性模型(OLS / Ridge / Lasso / ElasticNet),預測「之後」每支股票的報酬 —— 不偷看未來(防資料洩漏)。
4. **以 Ridge 為主**(headline):依 Ridge 預測分數挑最看好的 **前 20 支等權做多**,跟「全市場平均」比,算這一期賺賠;OLS / Lasso / ElasticNet 同步算 4 週期成績存進 JSON 之 `variants` 供並排比較。
...
7. 用門檻(§14.7-CZ T_CZ-6:有效 t 值 ≥ 4.20 且 Sharpe ≥ 2.40 且 勝率 ≥ 79%)判定:**這線性基準在哪個週期「真的能賺錢且可信」**。

**輸入**:資料庫(feature_values 特徵 + TaiwanStockPriceAdj 股價)。**輸出**:JSON(Ridge headline 各週期成績 + `variants` 收 OLS/Lasso/ElasticNet)+ log。
**它不做的事**:不訓練正式上線模型、不改資料庫(純讀取評估;§3.1 evaluation 角色)。
**為什麼需要它**:回答「複雜模型有沒有贏過簡單線性」的實證裁判 —— 這是先前缺漏的線性基準;所有模型用同一支標準、同窗、同指標 → 成績才能公平並排比較。
...
8. **[Linear Baseline]** (v0.1, 憲法 §14.7-DG common comparison baseline): 本程式為 simplest linear baseline(OLS / Ridge / Lasso / ElasticNet on standardized features);headline = Ridge;與 tree / transformer / foundation 共享同一 walk-forward / overlap correction / honest annualization protocol → 回答「複雜模型是否贏過線性」。
9. **[Zero Hardcoded Verdict]** (v0.1, 憲法 §5.6.3): 4 horizons 之 significance verdict 動態計算(`abs(eff_t_stat) > 1.997`,由 helper 計),**不**硬編 ✅/❌ 結論。
10. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切模組): 本程式為 **§14.7-CY Multi-Cycle Validation linear baseline 實作**(對標 multi_cycle_validation.py LGBM 為樹族第一實作)。**治權邊界**:(a) §3.2 evaluation 模組;(b) 五套禁令(§0.1-A / §0.2-A / §0.3-A / §0.0-E.4 / §6.8)不涉;(c) T1-T3 不分層;(d) §8.5 anti-leakage 不處理(features 已 anti-leakage compliant per feature_store_builder;train 僅用過去 panels);(e) **不訓練 production model**(僅 evaluate / 不寫 model_registry);(f) **不修改 feature_values / TaiwanStockPriceAdj**(read-only);(g) 唯一職責:跑 4-horizon walk-forward linear evaluation + 計算 metrics + 持久化 JSON。
11. **[Historical Reference Authority]** (v0.1): 本程式 `TOOL_VER = "v0.1"` 屬於記述性快照;tree-family baseline 對比值來自既有 `reports/multi_cycle_*` JSON(權威來源)。
12. **[Idempotency]** (v0.1): `--dry-run` 不寫 DB;`--commit` 寫 `evaluation_log`(若實作);JSON output path 含 timestamp,不覆寫舊結果。

## 📊 二、全量功能群矩陣 (The Ultimate Functional Group Matrix)

> 本程式作為 §14.7-CY Multi-Cycle linear baseline 實作,依「驗證面向」拆分為 5 大功能群;每群對應憲章治權契約。
...
### Group B. Walk-Forward Linear Training — internal per-horizon

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Expanding window training | `for i in range(1, len(panel_keys))`(train [0..i-1] → test i)| §14.7-CW T_CW-2 |
| B.2 StandardScaler + 4 linear models(seed=5422)| `StandardScaler` + OLS / Ridge / Lasso / ElasticNet | §14.7-DG baseline |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 Cross-cycle matrix table | stdout 彙總 4 horizons(Ridge headline)| §14.7-CY T_CY-6 |
| D.2 JSON persistence | `reports/multi_cycle_linear_<timestamp>.json` | §14.7-CY T_CY-1 |
| D.3 Meta context | `_meta`(tool_ver / sklearn_version / seed / run_at)| §一.10 source_traceability |
| D.4 variants(OLS/Lasso/ElasticNet)| JSON `variants` key 收 3 非 headline flavour | §14.7-DG comparable |
...
| 場景 | 對應命令 |
| :--- | :--- |
| 日常 multi-cycle validation | `python scripts/evaluation/multi_cycle_linear_validation.py --dry-run` |
| Commit 評估結果 | `python scripts/evaluation/multi_cycle_linear_validation.py --commit --output reports/<path>.json` |
| 自訂 horizons | `... --horizons 7,30,90,365` |
| 比對 tree-family baseline | 對比 `reports/multi_cycle_*` JSON |
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-06-04 | Codex | **首版:§14.7-DG common comparison baseline — linear baseline 實作**(clone multi_cycle_xgboost_validation.py v0.2 結構,僅換模型 train/predict + 標頭)。**功能 4 點**:(a) 4-horizon walk-forward(5/20/60/252d);(b) StandardScaler + 4 linear models(OLS / Ridge / Lasso / ElasticNet),headline = Ridge;(c) OLS/Lasso/ElasticNet 4-horizon metrics 存 JSON `variants` key;(d) 共用 `get_canonical_panel_dates` / `summarize_horizon_metrics`(§14.7-DE/DF §0.0-I 單一引用源)。**治權邊界**:read-only(不寫 model_registry / 不改 feature_values);全 (b) DB query + source-pure(`is_null_imputed IS NOT TRUE`);0 AI memory(per §一.10)。**Output**:stdout + JSON(`reports/multi_cycle_linear_<timestamp>.json`)。**v0.1 治權邊界**:不入憲(無新 doctrine,僅 §14.7-DG baseline implementation)。 | **ACTIVE** |
"""
...
def evaluate_horizon(cur, panels, horizon_days, universe, label):
    ...
    variant_names = list(build_models(SEED).keys())
    panel_pa = {name: [] for name in variant_names}  # §14.7-DF: (pred, actual) per panel per variant → 共用 helper(單一來源)
    ...
    for i in range(1, len(panel_keys)):
        ...
        for name, model in build_models(SEED).items():
            ...
            panel_pa[name].append((pred_te, y_test))

    results = {}
    for name in variant_names:
        r = summarize_horizon_metrics(label, horizon_days, panel_pa[name])  # §14.7-DF Canonical Metric SSOT(單一來源)
        if r is not None:
            results[name] = r
    ...
    if head is None:
        return None, results
    logger.info(f"  {label}({horizon_days}d) [{HEADLINE_VARIANT}]: Sharpe {head['sharpe']:+.3f} | Eff t {head['effective_t_stat']:+.3f} | Win {head['win_rate']*100:.1f}% | IC {head['mean_ic']:+.4f} | NET {head['annualized_simple_net']*100:+.1f}%/yr")
    logger.info(f"    precision: hit {head['precision_directional_hit_rate']*100:.1f}% | top-20 overlap {head['precision_top20_actual_overlap']*100:.1f}% | RMSE {head['precision_rmse']:.4f} | reliability IC-CoV {head['reliability_ic_stability_cov']:.4f}")
    for name in variant_names:
        if name == HEADLINE_VARIANT or name not in results:
            continue
        v = results[name]
        logger.info(f"    [{name}] Sharpe {v['sharpe']:+.3f} | Eff t {v['effective_t_stat']:+.3f} | Win {v['win_rate']*100:.1f}% | NET {v['annualized_simple_net']*100:+.1f}%/yr")
    return head, results


def main():
    global SEED
    parser = argparse.ArgumentParser(description=f"Multi-Cycle Linear Baseline Validation {TOOL_VER}")
    ...
    SEED = args.seed  # §一.10 #3: inject run seed into linear models
    ...
    import sklearn
    logger.info("="*100)
    logger.info(f"Multi-Cycle Linear Baseline Validation {TOOL_VER}(per §14.7-CY / §14.7-DG)")
    logger.info("="*100)
    logger.info(f"  Horizons: {horizon_labels}")
    logger.info(f"  sklearn ver: {sklearn.__version__}")
    logger.info(f"  Variants: ols / ridge(headline) / lasso / elasticnet")
    ...
    try:
        ...
        results = {}            # headline (ridge) per horizon
        variant_results = {}    # all variants per horizon
        t_global = time.monotonic()
        for label, days in horizon_labels:
            head, all_v = evaluate_horizon(cur, panels, days, universe, label)
            if head: results[label] = head
            if all_v: variant_results[label] = all_v

        logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix(Linear · headline {HEADLINE_VARIANT})\n{'='*100}")
        ...
        if args.output or args.commit:
            output_path = args.output or f"reports/multi_cycle_linear_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            output_results["variants"] = {
                name: {label: variant_results[label][name]
                       for label in variant_results if name in variant_results[label]}
                for name in build_models(SEED).keys() if name != HEADLINE_VARIANT
            }
            output_results["_meta"] = {
                "tool": "multi_cycle_linear_validation.py",
                "tool_ver": TOOL_VER,
                "model_family": "linear",
                "headline_variant": HEADLINE_VARIANT,
                "variant_alphas": {"ridge": RIDGE_ALPHA, "lasso": LASSO_ALPHA,
                                   "elasticnet_alpha": ELASTICNET_ALPHA, "elasticnet_l1_ratio": ELASTICNET_L1_RATIO},
