"""
multi_cycle_dlinear_validation.py v0.1 (DLinear / NLinear · Decomposition & Normalization Linear · AAAI 2023 / Zeng et al. · Multi-Cycle Stock-Price Validation · per CLAUDE.md §一.11 三段式)
================================================================================
**最後更新日期**: 2026-06-02
**主權狀態**: DLinear/NLinear(DECOMPOSITION / NORMALIZATION LINEAR / AAAI 2023)4-HORIZON WALK-FORWARD VALIDATION + §14.7-CY HORIZON-DOCTRINE 第二族(neural)+ §14.7-DC v0.18 SOURCE-PURE UNIVERSE + §一.10 SOURCE-TRACEABLE(全 DB)+ §一.10 #3 MULTI-RUN + 共同比較基準(COMMON COMPARISON BASELINE)新增實作 + §一.11 三段式合規 + §14.7-DE §0.0-I panel-date helper
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 DLinear(超簡單的「拆解 + 線性」時序模型) 序列模型,吃每支股票的「歷史價格序列」,預測未來報酬,評估「靠它選股能不能賺錢、準不準、可不可信」。
...
**DLinear 的特別之處**:它**不用 Transformer**,只用「移動平均拆出趨勢 + 殘差季節」再各接一個線性層(channel-independent,每支股票共用同一組權重)。AAAI 2023 論文證明這種極簡模型在許多時序預測上反而打敗 Transformer。

**輸入**:資料庫(股價序列)。**輸出**:JSON(各週期成績)+ log。
**它不做的事**:不改資料庫(純讀取評估;§3.1 evaluation 角色)。
**為什麼需要它**:序列/基礎模型路線的實證裁判,與樹模型並列比較(共同欄位對齊後);提供「極簡線性 baseline」對照 Transformer 系列。
...
1. **[Sovereignty Declaration]** (v0.1, 憲法 §3.2 橫切分析模組): 本程式為 evaluation 後處理工具,非序列落地鏈模組。
   **治權邊界**:(a) §3.2 evaluation 模組;(b) 不涉五套禁令(§0.1/§0.2/§0.3);(c) T1-T3 不分層;
   ...
   (g) 唯一職責:DLinear/NLinear 4-horizon walk-forward 預測 + 共同比較基準 metrics + JSON 持久化。
2. **[Common Comparison Baseline]** (v0.1, reports/common_model_comparison_baseline_v1.md): 本程式為共同比較基準之**新增實作 (DLinear)** —
   ...
   **完全相同** realized targets → 精準度(precision)/ 信任度(trust)比較 apples-to-apples。模型用各自 natural representation
   (tree=38 cross-sectional features;TFT=每股 weekly 序列 + GroupNormalizer pooled;DLinear=**channel-independent 每股 weekly return 序列**),
...
5. **[Real DLinear/NLinear — Decomposition/Normalization Linear]** (v0.1, Zeng/Chen/Zhang/Xu, AAAI 2023 "Are Transformers
   Effective for Time Series Forecasting?"): **channel-independent** — 同一組 linear 權重套用於每支股票之 length-L 序列。
   **DLinear(default)**:每段 L 序列以 moving-average(avg_pool1d, kernel=kernel_size, edge-pad 保長 L)分解為 trend + seasonal(=series−trend),
   兩個獨立 Linear(L→S),output = trend_linear(trend) + seasonal_linear(seasonal);**NLinear(--variant nlinear)**:減去序列最後一個 lookback 值,
   單一 Linear(L→S),再加回最後值 → 對 distribution shift 穩健。非 surrogate;從頭以 torch.nn 實作,與 multi_cycle_itransformer_validation.py 同慣例。
   padded/NaN 之 stock 於 loss masking 處理(模組本身可忽略 key_padding_mask)。
6. **[Point-Forecast → Calibration N/A]** (v0.1): DLinear/NLinear 原始為 point forecast(MSE loss),非 quantile 模型 →
...
9. **[§一.10 #3 Multi-Run]** (v0.1): stochastic(torch init / sgd shuffle)→ 須 ≥3 seeds {5422,7331,1009};
...
12. **[Idempotency]** (v0.1): 不寫 DB(--output 寫 JSON);可重跑;refit cadence / kernel size / variant / epochs / lookback 全可配置 → 跑期可控。
...
### Group B. DLinear/NLinear Model(channel-independent 每股 weekly return 序列 → forecast)

| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| B.1 Return matrix | load_return_matrix() → daily close → calendar-week → log-return matrix[week × stock]| §一.10 / §一.13 source-pure |
| B.2 Model | DLinear:moving-avg decompose(trend+seasonal)→ 兩 Linear(L→S);NLinear:last-value norm + 單 Linear(L→S)| Zeng et al. AAAI 2023 |
| B.3 Train | train_dlinear() → Adam + MSE(masked)+ grad-clip(CPU)| channel-independent linear |
...
| 子項 | 對應方法 / 行為 | 治權契約 |
| :--- | :--- | :--- |
| D.1 JSON | --output reports/dlinear_v0/<...>.json(schema-compatible + precision/trust)| §一.10 / §二.4 |
...
| 場景 | 命令 |
| :--- | :--- |
| Smoke(plumbing 驗證)| `python scripts/evaluation/multi_cycle_dlinear_validation.py --smoke --output reports/dlinear_v0/_smoke.json` |
| 完整單 seed | `... --seed 5422 --output reports/dlinear_v0/dl_s5422.json` |
...
# ════════════════════════════════════════════════════════════════════════════
# Group B — DLinear return-matrix construction (channel-independent per-stock series)
# ════════════════════════════════════════════════════════════════════════════
...
# ── DLinear/NLinear architecture (from-scratch torch.nn; channel-independent linear) ──
def _build_model(L, S, kernel_size, variant):
    import torch
    import torch.nn as nn
    import torch.nn.functional as F

    class DLinear(nn.Module):
        """Decomposition Linear (Zeng et al., AAAI 2023). Channel-independent:
        same weights across every stock's length-L series. forward(x:[B,N,L])."""
        def __init__(self):
            super().__init__()
            self.variant = variant
            self.kernel_size = kernel_size
            if variant == "nlinear":
                self.linear = nn.Linear(L, S)
            else:
                self.trend_linear = nn.Linear(L, S)
                self.seasonal_linear = nn.Linear(L, S)

        def _moving_avg(self, x):                            # x:[*, L] → trend:[*, L]
            pad_left = (self.kernel_size - 1) // 2
            pad_right = self.kernel_size - 1 - pad_left
            xp = F.pad(x.unsqueeze(1), (pad_left, pad_right), mode="replicate")  # edge-pad keeps length L
            trend = F.avg_pool1d(xp, kernel_size=self.kernel_size, stride=1)     # [*,1,L]
            return trend.squeeze(1)

        def forward(self, x, key_padding_mask=None):         # x:[B,N,L]  mask ignored (loss-masked upstream)
            B, N, L = x.shape
            xf = x.reshape(B * N, L)                          # channel-independent: each (b,n) series
            if self.variant == "nlinear":
                last = xf[:, -1:]                             # [B*N,1]
                out = self.linear(xf - last) + last          # subtract/add last lookback value
            else:
                trend = self._moving_avg(xf)                  # [B*N,L]
                seasonal = xf - trend
...
def train_dlinear(ret_mat, week_cut, L, S, seed, epochs, lr, kernel_size, variant,
                  batch_size, min_valid=N_TOP + 5):
    """Train DLinear/NLinear on all windows whose lookback+target ∈ weeks ≤ week_cut (leakage-safe).
    ...
    model = _build_model(L, S, kernel_size, variant)
...
def aggregate_horizon(label, horizon_days, panel_top_rets, panel_univ_rets,
    ...
    return {
        ...
        "calibration_p10_p90_coverage": calib_cov,  # None for DLinear (point forecast, non-quantile)
    }
...
def run(args):
    ...
    logger.info(f"DLinear refit at panel indices {refit_points} (every {refit_every}); "
                f"L={L}wk S={S}wk variant={args.variant} kernel={args.kernel_size} "
    ...
    for pi, as_of in enumerate(panels):
        ...
        if pi in refit_points or model is None:
            tr0 = time.monotonic()
            res = train_dlinear(ret_mat, wc, L, S, args.seed, args.epochs, args.lr,
                                args.kernel_size, args.variant, args.batch_size)
            if res is None:
...
def main():
    ap = argparse.ArgumentParser(description=f"Multi-Cycle DLinear Validation {TOOL_VER}")
    ...
    ap.add_argument("--kernel-size", type=int, default=25, help="moving-average kernel for DLinear decomposition")
    ap.add_argument("--variant", type=str, default="dlinear", choices=["dlinear", "nlinear"],
                    help="dlinear=moving-avg decompose; nlinear=last-value normalization")
    ...
    logger.info("=" * 100)
    logger.info(f"Multi-Cycle DLinear Validation {TOOL_VER} (Decomposition/Normalization Linear / AAAI 2023)")
    ...
    logger.info(f"\n{'='*100}\nCross-Cycle Comparison Matrix (DLinear)\n{'='*100}")
    ...
    if args.output:
        ...
        out["_meta"] = {
            "tool": "multi_cycle_dlinear_validation.py", "tool_ver": TOOL_VER,
            "model": "DLinear/NLinear (Decomposition/Normalization Linear, AAAI 2023, from-scratch torch.nn)",
            ...
            "variant": args.variant, "kernel_size": args.kernel_size,
            "refit_every": args.refit_every, "epochs": args.epochs, "smoke": args.smoke,
            "channels": "stocks (channel-independent linear)",
