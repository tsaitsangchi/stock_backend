...
3. **建表 + 補欄 + 加寬**(`ensure_table`):`CREATE TABLE IF NOT EXISTS`;**若表已存在,改讀 DB 的實際主鍵並沿用**(主鍵首建後就固定,避免之後用單股小樣本重新推導出更窄的主鍵而把資料覆蓋掉);缺欄則 `ALTER TABLE ADD COLUMN` 補上;**既有 VARCHAR 欄若後續 batch 出現更寬值,自動 `ALTER COLUMN TYPE` 加寬**(只加寬不縮窄,無資料損失——防截斷)。
...
1. **[Generic Auto-Schema SSOT]** (v1.0, §0.0-I): 全系統「從 API 回應推導 schema + 自動建表」之**唯一引用源**;呼叫端不得自行複製本邏輯。字串 ≥ VARCHAR(255)、數字 ≥ NUMERIC(20,6)(用戶 directive 下限;2026-06-08 字串下限 100→255),值超界自動加大;**既有表後續 batch 出現更寬值時,ensure_table 自動 `ALTER COLUMN TYPE` 加寬既有 VARCHAR 欄(v1.3;只加寬不縮窄)**。
...
| v1.2 | 2026-06-08 | Claude | **字串型別下限 `MIN_VARCHAR` 100→255**(用戶 directive「所有欄位字串型態最少要大於 varchar(255) 以上」):所有 generic-built 表之字串欄 ≥ `VARCHAR(255)`(值超界仍自動加大;maxlen > `TEXT_THRESHOLD`=4000 改 `TEXT`);docstring [Generic Auto-Schema SSOT] + 白話段同步;`finmind_generic_ingest.py` 因 `import core.generic_schema` 自動繼承新下限(無獨立常數)。**附帶**:修正 v1.1 漏改之 header stale「不管 FRED / FredData 由 data_schema DDL」(FRED 已 generic,FredData/fred_series 由本模組建)。 | SUPERSEDED |
| v1.3 | 2026-06-08 | Claude | **`ensure_table` 既有表自動加寬 VARCHAR 欄**(解全市場全史 sync 之 146 `TaiwanStockShareholding.note` 截斷):既有表分支新增 `_db_col_types()`(讀 information_schema 現有欄寬)+ `_parse_sql_type()`(解析目標型別);目標 `VARCHAR(n)` > 現有寬度 → `ALTER COLUMN TYPE VARCHAR(n)`;目標 `TEXT` → `ALTER COLUMN TYPE TEXT`(只加寬不縮窄,無資料損失)。根因:原 `ensure_table` 既有表分支僅補缺欄,後續 batch 出現超初始寬度值即 `StringDataRightTruncation`;與 v1.2 之 255 floor **互補**(255 floor 防新表初建 ≤255 截斷;auto-widen 防既有表 + 任意超界值)。py_compile PASS;補抓 86 股,`note` 欄 100→234。 | **ACTIVE** |
"""
