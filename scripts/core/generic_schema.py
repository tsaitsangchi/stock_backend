...
**它怎麼做(步驟)**:
1. **看資料推型別**(`infer_schema`):純數字→`NUMERIC`(至少 20,6,值很大自動加大);`YYYY-MM-DD` 樣貌→`DATE`(讓發布日比較等日期運算正確);`stock_id`/券商碼/`year`/`Time`→強制 `VARCHAR`(避免被當數字掉前導零或誤判);其餘→`VARCHAR`(至少 100,超長改 `TEXT`)。
...
**它不做的事(治權邊界)**:**不抓 API**(呼叫端負責 fetch/throttle/quota/斷點續傳——本模組只處理「資料 → schema → 表 → upsert」);**不算特徵、不選股**(自動建的表要當特徵仍須過 §14.7-DC source-pure gate);**不管 infra 表 / FRED**(`pipeline_execution_log`/`data_audit_log`/`FredData` 仍由 `data_schema.py` 持明確 DDL——它們非 FinMind 個股 dataset);**不產生任何 synthetic/impute 值**(NULL 即 NULL,§一.10)。
...
1. **[Generic Auto-Schema SSOT]** (v1.0, §0.0-I): 全系統「從 API 回應推導 schema + 自動建表」之**唯一引用源**;呼叫端不得自行複製本邏輯。字串 ≥ VARCHAR(100)、數字 ≥ NUMERIC(20,6)(用戶 directive 下限),值超界自動加大。
2. **[Sovereignty Declaration]** (v1.0, 憲法 §3.2 橫切 library): 本模組為 §3.2 橫切基礎設施 library(schema-provisioning helper),非 §3.1 序列模組。**治權邊界**:(a) §3.2 橫切 utility;(b) 五套禁令(§0.1-A/§0.2-A/§0.3-A/§0.0-E.4/§6.8)不涉;(c) T1-T3 不分層;(d) §8.5 anti-leakage 不處理(raw schema provisioning;發布日治權仍由 `data_schema.build_publication_date_gate` 持有);(e) **不選股不評分**;(f) **不算特徵**;(g) **不抓 API**(呼叫端持有);(h) **不管 infra/FRED 表 DDL**(由 `data_schema.py` 持有);(i) 唯一職責:rows → schema 推導 → 建表/補欄 → 主鍵偵測/重用 → 冪等 upsert。
