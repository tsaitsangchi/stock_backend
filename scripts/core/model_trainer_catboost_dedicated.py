"""
model_trainer_catboost_dedicated.py v0.1 (CatBoost Tree Production Trainer · §14.7-CW Tree Family 第九實作 dedicated · per CLAUDE.md §一.11 三段式入憲 · per Canonical Comparison Framework)
================================================================================
**最後更新日期**: 2026-05-29
**主權狀態**: CATBOOST TREE PRODUCTION + §14.7-CW TREE-FAMILY 第九實作 dedicated(對標 LGBM v0.2 / LightGBM dedicated / XGBoost dedicated / Ensemble / Random Forest / Extra Trees / 既存 model_trainer_catboost.py)+ §14.7-CS MODEL-TRAINING-LANDING + §14.7-CL 43-FEATURE CANONICAL + §一.10 SOURCE-TRACEABLE + §一.11 三段式合規 + §一.12 5-MIN-REPORTING-AWARE + CANONICAL COMPARISON FRAMEWORK
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 **CatBoost(dedicated 專版)** 訓練股價預測 production 模型(§14.7-CW Tree Family;walk-forward 訓練)。

**輸入 → 輸出**:feature_values(37 特徵)+ 核心股 → CatBoost(dedicated 專版) model artifact + registry 列

**為什麼需要它**:產生實際上線用的模型檔(與 multi_cycle validator 雙軌:trainer 產 artifact、validator 自評)。

## 📜 一、核心定義說明 (Core Definitions / The Constitution)

1. **[Real Tree Model Required]** (v0.1, 憲法 §14.7-CW T_CW-1): CatBoost(Categorical Boosting / Yandex 2018 Prokhorenkova et al.)為 **symmetric / oblivious tree GBT**(對比 LightGBM leaf-wise + XGBoost level-wise);**ordered boosting**(prevent target leakage)+ **native categorical handling via ordered target statistics** 為核心 essence。
2. **[Expanding Window Walk-Forward OOS]** (v0.1, 憲法 §14.7-CW T_CW-2): train [panel 0..i-1] → test panel i,完全對齊 LGBM v0.2 / LightGBM dedicated / XGBoost dedicated / 既存 CatBoost / Ensemble / Random Forest / Extra Trees 之 walk-forward 設定。
3. **[Conservative Hyperparameters]** (v0.1, §14.7-CW T_CW-4): iterations=200 / learning_rate=0.05 / depth=5 / l2_leaf_reg=3 / subsample=0.8 / colsample_bylevel=0.8 / min_data_in_leaf=30 / random_seed=5422,完全對齊既存 model_trainer_catboost.py v0.1 之 hyperparams。
...
7. **[Symmetric Tree + Ordered Boosting]** (v0.1, 對標 LightGBM/XGBoost 差異): CatBoost 之 symmetric(oblivious)tree 在每一 depth 用同一 split condition(more balanced + lower overfit),vs LightGBM leaf-wise / XGBoost level-wise 之 asymmetric splits;**ordered boosting** 在 training 中 per-instance 使用 historical-only model 防 target leakage(對比一般 GBT 同步用全 data)。
...
10. **[Sovereignty Declaration]** (v0.1, §3.1 序列模組): 本程式為 **§10 model_trainer 第九實作 dedicated**(LGBM v0.2 / 既存 XGBoost / 既存 CatBoost / Ensemble / Random Forest / Extra Trees / LightGBM dedicated / XGBoost dedicated 為前八)。**治權邊界**:(a) §3.1 序列 training,不入 §3.2 橫切;(b) 五套禁令不涉;(c) T1-T3 不分層(屬 L1 evidence);(d) §8.5 anti-leakage compliant;(e) 不選股 / 不算 feature;(f) **不評估 multi-cycle**(由 multi_cycle_catboost_dedicated_validation 負責);(g) 唯一職責:8-panel walk-forward CatBoost 訓練 + Treaty gates + artifact + model_registry。
11. **[Historical Reference Authority]** (v0.1): `TOOL_VER = "v0.1"`;**既存 CatBoost v0.1 為 §14.7-CW Tree Family 第三實作**(quarterly Eff t 3.65 / Sharpe 2.30 / NetAnn +20.22% / weekly only sig per §14.7-CY multi-cycle reference);本 dedicated v0.1 為 Canonical Comparison Framework 對齊版本,與既存並存。
12. **[Idempotency]** (v0.1): model_registry INSERT ON CONFLICT;重跑覆寫。
13. **[Canonical Comparison Framework]** (v0.1, per RF v0.1 建立): 本程式之 metrics 計算邏輯(Spearman IC / Sharpe / MDD / α / t-stat / overfit gap)與 LGBM v0.2 / LightGBM dedicated / XGBoost dedicated / 既存 XGBoost / 既存 CatBoost / Ensemble / Random Forest / Extra Trees 完全 standardized,確保 9-tree comparison reliable。
14. **[Multi-Run Reproducibility]** (v0.1, per §14.7-CW T_CW-6 v6.17.1 patch): CatBoost subsample(stochastic gradient boosting)+ random_strength 含 stochasticity;commit anchor 須註明為 single run / range;per §一.10 #3 production metric 應 ≥3 runs 取得 min/median/max statistics。
...
| B.2 CatBoost params | `DEFAULT_TRAINING_POLICY`(200/0.05/5/3/0.8/0.8/30/5422)| §14.7-CW T_CW-4 |
| B.3 Symmetric tree | CatBoost default `boosting_type='Plain'` + symmetric | overfit prevention |
...
| 子項 | 對應方法 | 治權契約 |
| :--- | :--- | :--- |
| D.1 native save | `model.save_model(.cbm)` | CatBoost standard |
...
| 子項 | 對應方法 | 治權契約 |
| :--- | :--- | :--- |
| E.1 Feature importance(PredictionValuesChange)| `model.get_feature_importance()` | §14.7-CN 4-path |
...
| 場景 | 命令 |
| :--- | :--- |
| 日常 dry-run | `python scripts/core/model_trainer_catboost_dedicated.py --dry-run` |
| Commit production | `python scripts/core/model_trainer_catboost_dedicated.py --commit` |
...
| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-05-29 | Codex | **首版:§14.7-CW Tree Family 第九實作 dedicated(CatBoost)** under Canonical Comparison Framework。 (1) **Naming alignment**:配合 LightGBM/XGBoost dedicated naming pattern;(2) **與既存 model_trainer_catboost.py(§14.7-CW v0.1 第三實作)並存**;此 dedicated 版本為 Canonical Comparison Framework 對齊版本;(3) Hyperparameters 完全對齊既存 v0.1(200/0.05/5/3/0.8/0.8/30/5422);(4) Canonical Comparison Framework 第九實作,延伸 8-tree → 9-tree。**新介面**:`--dry-run` / `--commit` / `--panel-feature-sets <csv>` / `--label-horizon N`(全與 LightGBM/XGBoost dedicated v0.1 對等)。 | **ACTIVE** |
"""
...
def main():
    parser = argparse.ArgumentParser(description=f"CatBoost Tree Model Trainer ({TOOL_VER}) — Canonical Comparison Framework")
    ...
    try:
        ...
        logger.info("=" * 120)
        logger.info(f"§14.7-CW CatBoost Tree Model Trainer {TOOL_VER}(Canonical Comparison Framework)")
        logger.info("=" * 120)
        ...
        if args.commit:
            ...
            model_id = f"mdl_{train_date.replace('-', '')}_catboost_dedicated_h{args.label_horizon}_{feature_set_hash}_v0_1"
            ...
            metrics = {
                "trainer": "catboost_dedicated_v0_1",
        ...
        logger.info(f"§14.7-CW CatBoost Dedicated Trainer {TOOL_VER}: 主權判定 {verdict}")
        logger.info("=" * 120)
        if verdict == "PERFECT":
            logger.info(f"  🎯 CatBoost(dedicated): Sharpe={sharpe:.2f} / IR={ir:.2f} / Win={win_rate*100:.0f}%")
    finally:
        conn.close()
