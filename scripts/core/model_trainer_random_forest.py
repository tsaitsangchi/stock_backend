"""
model_trainer_random_forest.py v0.1 (Random Forest Tree Production Trainer · §14.7-CW Tree Family 第五實作 · per CLAUDE.md §一.11 三段式入憲 · per Canonical Comparison Framework)
================================================================================
**最後更新日期**: 2026-05-29
...
2. **[Expanding Window Walk-Forward OOS]** (v0.1, 憲法 §14.7-CW T_CW-2): train [panel 0..i-1] → test panel i。
3. **[Conservative Hyperparameters]** (v0.1, §14.7-CW T_CW-4): n_estimators=200 / max_depth=5 / min_samples_leaf=30 / max_features='sqrt' / random_state=5422 / 對等 LGBM/XGBoost/CatBoost defaults。
4. **[Treaty Gates 4/4]** (v0.1, §14.7-CW T_CW-5): Sharpe > 0 / Win ≥ 50% / MDD ≤ 30% / Mean α > 0。
5. **[Top Features §0.1+§0.2 Dominated]** (v0.1, §14.7-CW T_CW-3): top-15 features 預期由 §0.1+§0.2 主導。
6. **[43 Canonical Features]** (v0.1, §14.7-CL): SPEC_43 與其他 tree models 共用。
7. **[Bagging vs Boosting]** (v0.1): Random Forest 之 bagging(bootstrap aggregating)為 variance reduction;GBT family 之 boosting 為 bias reduction;兩者 architectural complementary。
...
10. **[Sovereignty Declaration]** (v0.1, §3.1 序列模組): 本程式為 **§10 model_trainer 第五實作**(LGBM/XGB/CatBoost/Ensemble 為前四)。**治權邊界**:(a) §3.1 序列 training;(b) 五套禁令不涉;(c) T1-T3 不分層;(d) §8.5 已 anti-leakage compliant;(e) 不選股 / 不算 feature;(f) **不評估 multi-cycle**(由 multi_cycle_random_forest_validation 負責);(g) 唯一職責:8-panel walk-forward RF 訓練 + Treaty gates + artifact + model_registry。
11. **[Historical Reference Authority]** (v0.1): `TOOL_VER = "v0.1"`;LGBM v0.2 / XGBoost v0.1 / CatBoost v0.1 / Ensemble v0.1 為對標 reference。
12. **[Idempotency]** (v0.1): model_registry INSERT ON CONFLICT;重跑覆寫。
13. **[Canonical Comparison Framework]** (v0.1, 新建): 本程式之 metrics 計算邏輯與 LGBM/XGBoost/CatBoost/Ensemble 完全 standardized,確保 future model 對比 reliable。
...
| B.2 RF params | `DEFAULT_TRAINING_POLICY`(200 trees / depth 5 / seed 5422)| §14.7-CW T_CW-4 |
| B.3 Bagging | bootstrap=True(RF default)| variance reduction |
...
| 場景 | 命令 |
| :--- | :--- |
| 日常 dry-run | `python scripts/core/model_trainer_random_forest.py --dry-run` |
| Commit production | `python scripts/core/model_trainer_random_forest.py --commit` |

### 不提供之旗標 (Intentionally Omitted)
- `--seed`:固定 5422 per §14.7-CW T_CW-4。
- `--no-bootstrap`:bootstrap=True 為 RF essence。

## 📜 三、全修訂歷程 (Full Revision History)

| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-05-29 | Codex | **首版:§14.7-CW Tree Family 第五實作(Random Forest)**。bagging-based ensemble vs GBT family 之 boosting 互補。**Hyperparameters**:n_estimators=200 / max_depth=5 / min_samples_leaf=30 / max_features='sqrt' / random_state=5422 / n_jobs=-1。**Canonical Comparison Framework 新建**:metrics 計算邏輯與前四 tree models 完全 standardized。 | **ACTIVE** |
"""
...
def main():
    parser = argparse.ArgumentParser(description=f"Random Forest Tree Model Trainer ({TOOL_VER})")
    ...
    try:
        ...
        logger.info("=" * 120)
        logger.info(f"§14.7-CW Random Forest Tree Model Trainer {TOOL_VER}")
        logger.info("=" * 120)
        ...
        for i in range(1, len(panels_list)):
            ...
            fold_model = RandomForestRegressor(**DEFAULT_TRAINING_POLICY)
        ...
        model = RandomForestRegressor(**DEFAULT_TRAINING_POLICY)
        ...
        if args.commit:
            ...
            model_id = f"mdl_{train_date.replace('-', '')}_random_forest_h{args.label_horizon}_{feature_set_hash}_v0_1"
            ...
            """, (model_id, "model_policy_v0.1", MODEL_FAMILY, panels[-1][0], universe_snapshot_id, args.label_horizon, panels[0][1], train_date, json.dumps(metrics, default=str), json.dumps(DEFAULT_TRAINING_POLICY), str(artifact_dir), "committed", "v0.1 Random Forest;walk-forward;§14.7-CW family 5th implementation"))
            conn.commit()
        ...
        logger.info(f"§14.7-CW Random Forest Trainer {TOOL_VER}: 主權判定 {verdict}")
        logger.info("=" * 120)
        if verdict == "PERFECT":
            logger.info(f"  🎯 Random Forest: Sharpe={sharpe:.2f} / IR={ir:.2f} / Win={win_rate*100:.0f}%")
    finally:
        conn.close()
