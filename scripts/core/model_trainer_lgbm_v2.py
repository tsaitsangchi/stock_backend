"""
model_trainer_lgbm_v2.py — LightGBM real tree model trainer(§14.7-CW v6.17.0)
================================================================================
最後更新日期: 2026-05-28
治權: §14.7-CW Tree Model Production Upgrade(real LGBM tree replaces rank-IC baseline)

核心改進(對應 rank-IC baseline v0.1):
  1. Real LightGBM tree(non-linear interactions captured)
  2. 8-panel cross-validation training(增加 sample 至 ~9,000)
  3. TW regime-aware feature priority(theme_is_semiconductor binary split)
  4. Sector-balanced(per §14.7-AA Part C 100% concentration root cause 治本)
  5. Multi-panel forward labels(避免 single-point overfit)

目標(per §14.7-CV gates):
  - Sharpe ≥ 3.5(institutional+;baseline 3.10)
  - Win rate ≥ 75%
  - MDD ≤ 15%
  - Mean alpha > +8%
  - Cross-panel IC robust(>5/8 positive)

CLI:
  --dry-run / --commit
  --panel-feature-sets <fs1,...,fs8>(walk-forward training set)
  --label-horizon N(default 30)
"""
