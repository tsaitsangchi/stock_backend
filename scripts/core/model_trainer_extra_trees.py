"""
model_trainer_extra_trees.py v0.1 (Extra Trees Tree Production Trainer · §14.7-CW Tree Family 第六實作 · per CLAUDE.md §一.11 三段式入憲 · per Canonical Comparison Framework)
================================================================================
**最後更新日期**: 2026-05-29
**主權狀態**: EXTRA TREES TREE PRODUCTION + §14.7-CW TREE-FAMILY 第六實作(對標 LGBM/XGBoost/CatBoost/Ensemble/Random Forest)+ §14.7-CS MODEL-TRAINING-LANDING + §14.7-CL 43-FEATURE CANONICAL + §一.10 SOURCE-TRACEABLE + §一.11 三段式合規
**最高原則**: THE SUPREME AUTHORITY PRINCIPLE (最高權限原則)

## 🎯 零、這支程式在做什麼(白話說明,給人看的)

**一句話**:用 **Extra Trees** 訓練股價預測 production 模型(§14.7-CW Tree Family;walk-forward 訓練)。

**輸入 → 輸出**:feature_values(37 特徵)+ 核心股 → Extra Trees model artifact + registry 列

**為什麼需要它**:產生實際上線用的模型檔(與 multi_cycle validator 雙軌:trainer 產 artifact、validator 自評)。

## 📜 一、核心定義說明 (Core Definitions / The Constitution)

1. **[Real Tree Model Required]** (v0.1, 憲法 §14.7-CW T_CW-1): Extra Trees(Extremely Randomized Trees)為 bagging-based ensemble,split threshold 完全隨機(不搜索 best split),variance reduction 更強。
2. **[Expanding Window Walk-Forward OOS]** (v0.1, 憲法 §14.7-CW T_CW-2): train [panel 0..i-1] → test panel i,完全對齊 LGBM/XGBoost/CatBoost/Ensemble/Random Forest 之 walk-forward 設定。
3. **[Conservative Hyperparameters]** (v0.1, §14.7-CW T_CW-4): n_estimators=200 / max_depth=5 / min_samples_leaf=30 / max_features='sqrt' / random_state=5422,對等其他 tree models defaults。
4. **[Treaty Gates 4/4]** (v0.1, §14.7-CW T_CW-5): Sharpe > 0 / Win ≥ 50% / MDD ≤ 30% / Mean α > 0 完全對標其他 tree models。
...
7. **[Random-Split Bagging]** (v0.1, 對標 Random Forest 差異): Extra Trees 之 split 完全隨機(不搜索 best),vs Random Forest 之 best-split-within-random-subset;變異更大、bias 略增、variance 進一步減少。
...
10. **[Sovereignty Declaration]** (v0.1, §3.1 序列模組): 本程式為 **§10 model_trainer 第六實作**(LGBM/XGB/CatBoost/Ensemble/Random Forest 為前五)。**治權邊界**:(a) §3.1 序列 training,不入 §3.2 橫切;(b) 五套禁令不涉(不寫 F=M×ΔlnP);(c) T1-T3 不分層(屬 L1 evidence);(d) §8.5 anti-leakage compliant;(e) 不選股 / 不算 feature(SPEC_43 由 feature_store_builder 出);(f) **不評估 multi-cycle**(由 multi_cycle_extra_trees_validation 負責);(g) 唯一職責:8-panel walk-forward Extra Trees 訓練 + Treaty gates + artifact + model_registry。
11. **[Historical Reference Authority]** (v0.1): `TOOL_VER = "v0.1"`;LGBM v0.2 / XGBoost v0.1 / CatBoost v0.1 / Ensemble v0.1 / Random Forest v0.1 為對標 reference,非權威來源。
12. **[Idempotency]** (v0.1): model_registry INSERT ON CONFLICT;重跑覆寫。
13. **[Canonical Comparison Framework]** (v0.1, per RF v0.1 建立): 本程式之 metrics 計算邏輯(Spearman IC / Sharpe / MDD / α / t-stat / overfit gap)與 LGBM/XGBoost/CatBoost/Ensemble/Random Forest 完全 standardized。
14. **[ET vs RF Pairwise Ablation]** (v0.1, 對標研究): Extra Trees + Random Forest 為 bagging 家族 pairwise ablation;random-split vs best-split-in-subset 之 architectural choice 對 Tree Family production 之影響。
...
| B.2 ET params | `DEFAULT_TRAINING_POLICY`(200 trees / depth 5 / seed 5422)| §14.7-CW T_CW-4 |
...
| 子項 | 對應方法 | 治權契約 |
| :--- | :--- | :--- |
| D.1 joblib save | `joblib.dump(model, .pkl)` | sklearn standard |
...
| 子項 | 對應方法 | 治權契約 |
| :--- | :--- | :--- |
| E.1 Feature importance(MDI)| `model.feature_importances_` | §14.7-CN 4-path |
...
| 場景 | 命令 |
| :--- | :--- |
| 日常 dry-run | `python scripts/core/model_trainer_extra_trees.py --dry-run` |
| Commit production | `python scripts/core/model_trainer_extra_trees.py --commit` |

### 不提供之旗標 (Intentionally Omitted)
- `--seed`:固定 5422 per §14.7-CW T_CW-4(seed-aware reproducibility)。
- `--no-bootstrap`:bootstrap=True 為對齊 RF 之 bagging 對等性。

## 📜 三、全修訂歷程 (Full Revision History)

| 版本 | 日期 | 修訂者 | 修訂說明 | 治權狀態 |
| :--- | :--- | :--- | :--- | :--- |
| v0.1 | 2026-05-29 | Codex | **首版:§14.7-CW Tree Family 第六實作(Extra Trees)**。 (1) bagging + random-split,vs RF 之 best-split-within-subset 之 pairwise ablation;(2) Hyperparameters:n_estimators=200 / max_depth=5 / min_samples_leaf=30 / max_features='sqrt' / random_state=5422 / bootstrap=True / n_jobs=-1;(3) Canonical Comparison Framework 第六實作,延伸 5-tree comparison 為 6-tree;(4) ET vs RF pairwise ablation 揭露 variance-bias tradeoff effect。**新介面**:`--dry-run` / `--commit` / `--panel-feature-sets <csv>` / `--label-horizon N`(全與 RF v0.1 對等)。 | **ACTIVE** |
"""
...
def main():
    parser = argparse.ArgumentParser(description=f"Extra Trees Tree Model Trainer ({TOOL_VER})")
    ...
    try:
        ...
        logger.info("=" * 120)
        logger.info(f"§14.7-CW Extra Trees Tree Model Trainer {TOOL_VER}")
        logger.info("=" * 120)
        ...
        for i in range(1, len(panels_list)):
            ...
            fold_model = ExtraTreesRegressor(**DEFAULT_TRAINING_POLICY)
        ...
        model = ExtraTreesRegressor(**DEFAULT_TRAINING_POLICY)
        ...
        if args.commit:
            ...
            model_id = f"mdl_{train_date.replace('-', '')}_extra_trees_h{args.label_horizon}_{feature_set_hash}_v0_1"
            ...
            """, (model_id, "model_policy_v0.1", MODEL_FAMILY, panels[-1][0], universe_snapshot_id, args.label_horizon, panels[0][1], train_date, json.dumps(metrics, default=str), json.dumps(DEFAULT_TRAINING_POLICY), str(artifact_dir), "committed", "v0.1 Extra Trees;walk-forward;§14.7-CW family 6th implementation;ET vs RF pairwise ablation"))
            conn.commit()
        ...
        logger.info(f"§14.7-CW Extra Trees Trainer {TOOL_VER}: 主權判定 {verdict}")
        logger.info("=" * 120)
        if verdict == "PERFECT":
            logger.info(f"  🎯 Extra Trees: Sharpe={sharpe:.2f} / IR={ir:.2f} / Win={win_rate*100:.0f}%")
    finally:
        conn.close()
