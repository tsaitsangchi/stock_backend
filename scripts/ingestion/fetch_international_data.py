"""
fetch_international_data.py v4.3 (Quantum Finance Edition)
================================================================================
國際股市抓取器 — 美股連動專項 (Quantum v5.1 標準)
負責同步美股 (TSM, NVDA, AAPL 等) 的日線數據，作為台股盤前參考。
...
【執行範例矩陣 — 數據抓取方案】
1. 單一美股、單一表格同步 (Python)：
   python scripts/ingestion/fetch_international_data.py --stock_id TSM
2. 單一美股、單一表格「強制」更新歷史 (Python)：
   python scripts/ingestion/fetch_international_data.py --stock_id TSM --force
...
def sync_international(stock_id: str, start_date: str = None):
    start_time = time.time()
    client = FinMindClient()
    TABLE_NAME = "us_stock_price"
    
    if not start_date:
        last_date = get_latest_date(TABLE_NAME, stock_id) or "2010-01-01"
        start_date = (datetime.strptime(last_date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
    
    logger.info(f"🇺🇸 正在同步美股數據: {stock_id} (Since: {start_date})...")
    
    try:
        raw_data = client.get_data("USStockPrice", stock_id, start_date)
        if raw_data:
            ...
            # 🔴 混合模式日誌
            write_pipeline_log("FetchIntl", stock_id, "SUCCESS", "Ingestion", duration_ms=duration, rows=rows)
        ...
        write_pipeline_log("FetchIntl", stock_id, "FAILED", "Ingestion", err=str(e))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--stock_id", type=str, default="TSM")
    parser.add_argument("--start_date", type=str)
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    sync_international(args.stock_id, args.start_date if not args.force else "2010-01-01")