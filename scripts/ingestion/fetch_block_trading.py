"""
fetch_block_trading.py v4.3 (Quantum Finance Edition)
================================================================================
鉅額交易抓取器 — 核心行情補充 (Quantum v5.1 標準)
負責同步個股的鉅額交易數據，包括成交量、價格與交易類型。
...
2. 單一標的、單一表格「強制」更新歷史 (Python)：
   python scripts/ingestion/fetch_block_trading.py --stock_id 2330 --force
3. 單一標的「所有」維度表格抓取 (透過編排器)：
   python scripts/ingestion/parallel_fetch.py --stock_id 2330 --table ALL
4. 核心標的集「所有」維度表格同步 (透過編排器)：
   python scripts/ingestion/parallel_fetch.py --universe core --table BlockTrading
5. 全市場標的「所有」維度表格同步 (透過編排器)：
   python scripts/ingestion/parallel_fetch.py --universe all --table BlockTrading
================================================================================
"""
...
def sync_block_trading(stock_id: str, start_date: str = None):
    ...
    logger.info(f"🚚 正在同步 {stock_id} 鉅額交易資料 (Since: {start_date})...")
    
    try:
        raw_data = client.get_data("TaiwanStockBlockTrading", stock_id, start_date)
        if raw_data:
            df = pd.DataFrame(raw_data)
            df.columns = [c.lower() for c in df.columns]
            unique_cols = ["date", "stock_id", "securities_trader_id", "price", "trade_type"]
            rows = bulk_upsert(TABLE_NAME, df.to_dict('records'), unique_cols=unique_cols)
            duration = int((time.time() - start_time) * 1000)
            
            # 🔴 混合模式日誌
            write_pipeline_log("FetchBlockTrading", stock_id, "SUCCESS", "Ingestion", duration_ms=duration, rows=rows)
            write_data_audit_log(TABLE_NAME, stock_id, start_date, datetime.now().strftime("%Y-%m-%d"), rows)
            
            logger.info(f"✅ {stock_id} 鉅額交易同步完成，筆數: {rows}")
        else:
            logger.info(f"ℹ️  {stock_id} 無新資料")
    except Exception as e:
        logger.error(f"❌ {stock_id} 鉅額交易同步失敗: {e}")
        write_pipeline_log("FetchBlockTrading", stock_id, "FAILED", "Ingestion", err=str(e))
...
if __name__ == "__main__":
    ...
    sync_block_trading(args.stock_id, args.start_date if not args.force else "2010-01-01")
