"""
fetch_sponsor_chip_data.py v4.3 (Quantum Finance Edition)
================================================================================
分點籌碼抓取器 — 八大行庫專項 (Quantum v5.1 標準)
負責同步個股的八大官股行庫買賣超數據，作為護盤與市場信心指標。
...
2. 單一標的、單一表格「強制」更新歷史 (Python)：
   python scripts/ingestion/fetch_sponsor_chip_data.py --stock_id 2330 --force
...
def sync_sponsor(stock_id: str, start_date: str = None):
    start_time = time.time()
    client = FinMindClient()
    TABLE_NAME = "eight_banks_buy_sell"
    
    if not start_date:
        last_date = get_latest_date(TABLE_NAME, stock_id) or "2020-01-01"
        start_date = (datetime.strptime(last_date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
    
    logger.info(f"🏛️ 正在同步 {stock_id} 八大行庫籌碼 (Since: {start_date})...")
    
    try:
        raw_data = client.get_data("TaiwanStockGovernmentControlsBuySell", stock_id, start_date)
        if raw_data:
            ...
            # 🔴 混合模式日誌
            write_pipeline_log("FetchSponsorChip", stock_id, "SUCCESS", "Ingestion", duration_ms=duration, rows=rows)
            write_data_audit_log(TABLE_NAME, stock_id, start_date, datetime.now().strftime("%Y-%m-%d"), rows)
            
            logger.info(f"✅ {stock_id} 八大行庫籌碼同步完成，筆數: {rows}")
        else:
            logger.info(f"ℹ️  {stock_id} 無新資料")
    except Exception as e:
        logger.error(f"❌ {stock_id} 八大行庫籌碼同步失敗: {e}")
        write_pipeline_log("FetchSponsorChip", stock_id, "FAILED", "Ingestion", err=str(e))
...
if __name__ == "__main__":
    ...
    sync_sponsor(args.stock_id, args.start_date if not args.force else "2010-01-01")