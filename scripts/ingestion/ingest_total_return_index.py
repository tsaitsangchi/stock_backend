"""
fetch_total_return_index.py v4.3 (Quantum Finance Edition)
================================================================================
報酬指數抓取器 — 績效基準專項 (Quantum v5.1 標準)
負責同步台股大盤報酬指數，作為策略回測的基準 (Benchmark)。
...
2. 市場指標、單一表格「強制」更新歷史 (Python)：
   python scripts/ingestion/fetch_total_return_index.py --start_date 2010-01-01
...
def sync_return_index(start_date: str = None):
    start_time = time.time()
    client = FinMindClient()
    TABLE_NAME = "total_return_index"
    
    if not start_date:
        last_date = get_latest_date(TABLE_NAME) or "2010-01-01"
        start_date = (datetime.strptime(last_date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
    
    logger.info(f"📊 正在同步大盤報酬指數 (Since: {start_date})...")
    
    try:
        raw_data = client.get_data("TaiwanStockTotalReturnIndex", "", start_date)
        if raw_data:
            ...
            # 🔴 混合模式日誌
            write_pipeline_log("FetchReturnIndex", "MARKET", "SUCCESS", "Ingestion", duration_ms=duration, rows=rows)
            write_data_audit_log(TABLE_NAME, "MARKET", start_date, datetime.now().strftime("%Y-%m-%d"), rows)
            
            logger.info(f"✅ 報酬指數同步完成，筆數: {rows}")
        else:
            logger.info(f"ℹ️  無新資料")
    except Exception as e:
        logger.error(f"❌ 報酬指數同步失敗: {e}")
        write_pipeline_log("FetchReturnIndex", "MARKET", "FAILED", "Ingestion", err=str(e))
...
if __name__ == "__main__":
    ...
    sync_return_index(args.start_date)