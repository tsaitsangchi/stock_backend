"""
fetch_month_revenue.py v4.3 (Quantum Finance Edition)
================================================================================
月營收數據抓取器 — 營收專項 (Quantum v5.1 標準)
負責同步個股的每月營收數據，作為基本面領先指標。
...
1. 單一標的、單一表格同步 (Python)：
   python scripts/ingestion/fetch_month_revenue.py --stock_id 2330
2. 單一標的、單一表格「強制」更新歷史 (Python)：
   python scripts/ingestion/fetch_month_revenue.py --stock_id 2330 --force
...
def sync_revenue(stock_id: str, start_date: str = None):
    ...
    logger.info(f"📈 正在同步 {stock_id} 月營收資料 (Since: {start_date})...")
    
    try:
        raw_data = client.get_data("TaiwanStockMonthRevenue", stock_id, start_date)
        if raw_data:
            ...
            # 🔴 混合模式日誌
            write_pipeline_log("FetchRevenue", stock_id, "SUCCESS", "Ingestion", duration_ms=duration, rows=rows)
            write_data_audit_log(TABLE_NAME, stock_id, start_date, datetime.now().strftime("%Y-%m-%d"), rows)
            
            logger.info(f"✅ {stock_id} 月營收同步完成，筆數: {rows}")
        else:
            logger.info(f"ℹ️  {stock_id} 無新資料")
    except Exception as e:
        logger.error(f"❌ {stock_id} 同步失敗: {e}")
        write_pipeline_log("FetchRevenue", stock_id, "FAILED", "Ingestion", err=str(e))
...
if __name__ == "__main__":
    ...
    sync_revenue(args.stock_id, args.start_date if not args.force else "2010-01-01")
