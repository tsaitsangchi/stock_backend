...
"""
fetch_sponsor_chip_data.py v3.1 — Sponsor 進階籌碼（八大行庫月批次優化版）
================================================================================
v3.1 速度優化：
  ★ eight_banks 由「逐單日抓全市場」改為「30 天月批次抓取」：
    從 2021 至今約 1,300+ 個交易日 × 1.2s delay → ≈ 26 分鐘
    改為 ~50 次月批次 → ≈ 1 分鐘，省 25× 時間
  ★ DB 寫入仍走 commit_per_stock_per_day，逐 (stock_id, date) 原子落盤不變
...
EIGHT_BANKS_CHUNK_DAYS = 30  # v3.1：每次抓取的天數區段
...
def fetch_eight_banks(conn, stock_ids, start, end, delay, force):
    ...
    e_dt = datetime.strptime(end, "%Y-%m-%d").date()
    if s_dt > e_dt:
        logger.info(f"  [eight_banks] 起始日 {s_dt} 已超過結束日 {e_dt}，無新資料")
        flog.summary()
        return
    ...
    # v3.1：30 天月批次抓取（FinMind 此 dataset 支援 end_date）
    curr = s_dt
    while curr <= e_dt:
        ...
        try:
            ...
            if data:
                # 按 (date, stock_id) 彙總（同日同股票多筆累加 buy/sell）
                agg = defaultdict(lambda: [0, 0])
                for r in data:
                    sid = r.get("stock_id")
                ...
                if agg:
                    ...
                    res = commit_per_stock_per_day(
                        conn, UPSERT_EIGHT_BANKS, rows,
                        "(%s, %s, %s, %s)",
                        label_prefix="eight_banks", failure_logger=flog,
                    )
                    total_rows += sum(res.values())
