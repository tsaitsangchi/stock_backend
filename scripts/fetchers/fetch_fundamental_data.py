"""
fetch_fundamental_data.py v5.1 (Trinity Core Edition)
================================================================================
...
核心特色：
  ...
  · 非法 ID 過濾     ─ 自動跳過非個股 ID (如 Automobile, Semiconductor)，根治斷路器誤觸。
...
修訂歷程：
  v5.1 (2026-05-09):
    ...
    - [監控] 補齊 write_fetch_log 的 rows_inserted 參數，確保監控數據完整。
  v5.0 (2026-05-09):
    - [核心] 修正 ImportError，全面換裝 db_transaction 與並發架構。

執行範例：
    # 範例 1：並發抓取全市場財報並使用 5 執行緒
    python scripts/fetchers/fetch_fundamental_data.py --all --workers 5
    
    # 範例 2：針對特定標的強制重補歷史財報
    python scripts/fetchers/fetch_fundamental_data.py --stock-id 2330 --start 2020-01-01 --force
"""
    ...
    with db_transaction() as cur:
        try:
            ...
            if pk_cols and "origin_name" not in pk_cols:
                logger.warning("🛠️ 偵測到舊版 Primary Key，正在自動升級...")
        ...
        except Exception as e: logger.debug(f"Schema 升級略過: {e}")
...
def fetch_one_stock(sid: str, start: str, end: str, force: bool) -> Tuple[str, int, int]:
    ...
    if not is_valid_stock_id(sid):
        write_fetch_log(table, sid, "skipped", "fund_v5.1", start, end, 0, 0, "invalid_stock_id")
        return sid, 0, 0
    ...
    if not force:
        ...
        if latest:
            ...
            if cur_start > end:
                write_fetch_log(table, sid, "skipped", "fund_v5.1", str(latest), end, 0, 0, "up_to_date")
                return sid, 0, 0
    ...
    try:
        ...
        while start_dt <= end_dt:
            ...
            if data:
                ...
                if records:
                    ...
                    total_error += error
                    write_fetch_log(table, sid, "success" if error == 0 else "partial", "fund_v5.1", seg_start, seg_end, duration_ms, success, None)
                else: write_fetch_log(table, sid, "no_new_data", "fund_v5.1", seg_start, seg_end, duration_ms, 0, None)
            else: write_fetch_log(table, sid, "no_new_data", "fund_v5.1", seg_start, seg_end, duration_ms, 0, None)
        ...
        logger.error(f"  ❌ {sid} 抓取中斷: {e}")
        write_fetch_log(table, sid, "failed", "fund_v5.1", cur_start, end, 0, 0, str(e))
        return sid, 0, 0

def main():
    parser = argparse.ArgumentParser(description="Fundamental Data Fetcher v5.1 (Trinity Core Edition)")
    ...
    logger.info("=" * 70)
    logger.info(f"  Fundamental Fetcher v5.1  ({datetime.now():%Y-%m-%d %H:%M:%S})")
    logger.info("=" * 70)
