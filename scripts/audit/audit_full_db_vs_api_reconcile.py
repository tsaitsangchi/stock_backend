...
2. **[Schema SSOT Alignment]** (v0.1, §0.0-I): 表 columns / unique-key 全部 import 自 `core.data_schema.DATASET_REGISTRY` + `FINMIND_API_TABLES`,**不自行 hardcode 任何欄位名**(避免 AI 幻像 schema)。
...
| B.2 DB 全史 load | `load_db_rows()` 依 DATASET_REGISTRY columns | §0.0-I SSOT |
...
PERSTOCK_TABLES = [t for t in FINMIND_API_TABLES if t != "TaiwanStockInfo"]
