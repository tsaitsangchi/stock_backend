...
- generated_at: 2026-05-18 14:46:14
...
- **PASS** `committed_model_cardinality`: current prediction-backed model=mdl_20260425_lgbm_h20_d969ffb1_v0_1; committed_model_count=48; historical walk-forward models allowed
- **PASS** `model_id_governance`: all committed model_ids include feature_set_version hash: count=48
- **PASS** `label_horizon`: all committed models horizon ∈ {20/30}: count=48
- **PASS** `model_data_cutoff`: historical label_date_max <= db max_price_date=2026-05-15; production-current uses required_label_date gate: historical=48, production_current=0
- **PASS** `model_quality`: all committed models have ic_mean > 0: count=48; current_trainer=robust_rank_ic_baseline_v0.1
- **PASS** `model_artifact`: all committed model artifacts exist: count=48
