...
**最後更新**：2026-06-02(§一.16 Canonical Panel Source（panel 窗單一引用源 `get_canonical_panel_dates()`，§14.7-DE）+ §一.17 Canonical Horizon Metric SSOT（metric 計算單一引用源 `summarize_horizon_metrics()`，§14.7-DF）雙 helper 入憲[用戶 directive「所有模型符合 §0.0-I 單一引用源 + 反硬編精神,入憲章」+ 2 AskUserQuestion（全 12 支切換 / 抽共用 metric helper）;雙 helper 落 `core/db_utils.py` v2.50;13 validator panel-date 切換（9 metric-helper / 4 torch deferred）;helper 忠實性 synthetic unit test bit-identical;LightGBM #1 + XGBoost #2 annual 過 T_CZ-6（NET +30.2%/+32.7%）;13 validator 標頭 v0.1→v0.2 §一.11 補正;雙層治權鎖 主憲章 §14.7-DE/§14.7-DF + §一.16/§一.17 同次入憲;已 commit+push 封存 v6.26.3]；前序 §一.13 v0.16 Membership Eligibility-Flag Binding Corrective 入憲[用戶授權「把這次 ② writer fix + root cause 入憲(§14.7-DC 補一條 membership eligibility-flag binding,類比 T_DC-27)」;Finding:committed doctrine-native snapshot 398 core 全 train_eligible/predict_eligible/backtest_eligible/downstream_ready=FALSE,與 §14.7-DC 治權契約 2 invariant「三 gate 皆通過 → core_tier='core_universe' / train_eligible=True」矛盾;root cause:`core_universe_builder.py` `_commit_doctrine_native` core membership INSERT 只寫 6 欄省略 4 個 eligibility/readiness 欄位 → 落 schema column DEFAULT FALSE;corrective:writer code fix 補 4 欄位 4×TRUE 對齊 SSOT `build_doctrine_gate_universe.py:280`[py_compile pass];**非 production 阻斷**(downstream trainer 以 `core_tier='core_universe'` 選股,非 train_eligible,訓練從未被擋);committed snapshot FALSE flags **不 hand-patch** per [[no-manual-data-fill]] — 下次 rebuild 自然產出正確 flags;**無 git commit + 無 retrain** 之 honest disclosure(§一.8);T_DC-29 Membership Eligibility-Flag Binding Treaty 入憲;雙層治權鎖 主憲章 §14.7-DC v0.16 + §一.13 v0.16 同次入憲];前序 §一.13 v0.15 amihud Dead-Feature Removal 入憲[用戶授權「移除 amihud(最乾淨)→ SPEC 38 → 37」+ 兩 AskUserQuestion 確認「全系統一致」/「刪除 amihud rows」;amihud = zero-variance dead feature **非 AI 幻像**,398 core DB-verified distinct=1 / 全 0.00000000;全 28 code files surgical removal[py_compile pass]+ DB v0.5-family DELETE 95840 rows + feature_count 38→37[guarded-transaction];T_DC-28 Dead-Feature Removal Treaty 入憲;既有 model 未 retrain + 無 git commit 之 honest disclosure;雙層治權鎖 主憲章 §14.7-DC v0.15 + §一.13 v0.15 同次入憲];前序 §一.13 v0.14 Trainer-Feature-Set-Binding Corrective 入憲[用戶授權 Task C「修正 code + 入憲」,**非 strictest re-check 循環**];Finding:§14.7-DC 治權契約 3「model_trainer inherent 已對齊」為 incomplete —— universe-membership 繼承乾淨 ≠ feature-value-source 乾淨;10 production trainers 之 `DEFAULT_PANELS` 原預設 legacy `feature_set_v0_4`(含 5 已移除 AI 幻像 feature + 3 imputed core)→ repoint recent-8 v0_5 + 載入 query 補 `is_null_imputed IS NOT TRUE` 防禦過濾[10/10 grep + py_compile 驗證];committed snapshot + 1001 membership rows explicit bind `feature_set_v0_5`[psql 0 residual NULL 驗證];8 v0_5 panel source-purity psql 驗證 398 core / 38 features / 0 imputed each;T_DC-27 Trainer-Feature-Set-Binding Treaty 入憲;heavy retrain 未授權不 auto-run[§一.12 + §二.6];雙層治權鎖 主憲章 §14.7-DC v0.14 + CLAUDE.md §一.13 v0.14 同次入憲)
...
#### 第零段:🎯 白話功能說明 (Plain-Language Purpose — 強制置於三段式最前)(2026-06-02 用戶 explicit directive 入憲)

任何 script 標頭在「核心定義說明」之前**必須**先有一段**讓非專家也看得懂**的白話功能說明(標題 `## 🎯 零、這支程式在做什麼(白話說明,給人看的)`),內容須含:

- **一句話**:這支程式到底在做什麼(用日常語言,**不堆治權術語 / §代號 / "第 N 實作 dedicated"**)。
- **它怎麼做(步驟)**:編號 list,白話描述資料流(吃什麼資料 → 做什麼 → 算什麼 → 得到什麼結論)。
- **輸入 / 輸出**:具體資料來源與產物。
- **它不做的事**:治權邊界(避免誤解)。
- **為什麼需要它**:這支程式存在的目的。

**動因**:用戶 2026-06-02 explicit「**具體要做什麼的功能也沒寫清楚…我根本都看不懂這程式是要做什麼的**」之直接入憲。原三段式之「核心定義說明」常被寫成治權術語(§14.7-X / T_xx 代號 / "第七實作 dedicated")→ **讀的人不知程式在幹嘛**;第零段強制白話前言,治權術語保留於第一段之後。**precedent**:13 支 `multi_cycle_*_validation.py` 原僅有治權核心定義 → 本次補白話段(用戶「全 13 支套用 + 入憲」directive,AskUserQuestion 確認)。違反者(新 script 無白話段、或白話段堆術語讓人看不懂)視為 §一.11 治權違規,須補正後才可 commit。
...
**v0.16 Membership Eligibility-Flag Binding Corrective(2026-05-31 用戶授權「把這次 ② writer fix + root cause 入憲(§14.7-DC 補一條 membership eligibility-flag binding,類比 T_DC-27)」入憲 — `core_universe_builder._commit_doctrine_native` core membership INSERT 省略 4 個 eligibility 欄位 → committed 398 core 全 train_eligible=FALSE,與治權契約 2 invariant 矛盾;writer code fix 補 4×TRUE + T_DC-29 Membership Eligibility-Flag Binding Treaty)**:

> ⚠️ **code-level writer fix(非 hand-patch DB)+ 非 production 阻斷**(per §一.8 honest labeling):corrective 僅修 writer code,committed snapshot 之 FALSE flags **不手動 UPDATE**(per [[no-manual-data-fill]]);下游 trainer 以 `core_tier='core_universe'` 選股(契約 3 / T_DC-2),非 `train_eligible` → 訓練從未被擋,係 dormant charter-vs-DB inconsistency 而非 active bug。

**Finding(DB-verified,isolated psql READ-ONLY,2026-05-31)**:committed snapshot `core_universe_20260529_..._policy_v0_18...` 之 398 core 全 `train_eligible / predict_eligible / backtest_eligible / downstream_ready = FALSE`(core_count=398 / train_eligible TRUE=0 / downstream_ready TRUE=0)。§14.7-DC 治權契約 2 明文 invariant「三 gate 皆通過 → train_eligible=True」→ **writer 違反契約 2**(charter doctrine vs committed DB divergence)。TRUE flags 現僅存 deprecated(200 rows)+ superseded(3724 rows)舊 non-doctrine-native snapshots。

**Root cause**:`core_universe_builder.py:2653-2658` core membership INSERT 僅寫 6 欄位,省略 4 個 eligibility 欄位 → 落 schema column DEFAULT FALSE(`core_universe_schema.py:133-136` 4 個皆 `BOOLEAN DEFAULT FALSE`)。quarantine INSERT(L2662-2669)反而明設前 3 = FALSE。權威 SSOT `build_doctrine_gate_universe.py:274-281` core 列明設 4×TRUE → `_commit_doctrine_native` 為唯一遺漏者。

**Corrective(writer code fix LANDED;DB 未 patch)**:`core_universe_builder.py:2653` core INSERT 補 `train_eligible, predict_eligible, backtest_eligible, downstream_ready` 明設 4×TRUE,對齊 `build_doctrine_gate_universe.py:280`;`executemany` 參數元組不變(新欄位 literal)。py_compile PASS。下次 doctrine-native rebuild(§14.7-DD PHASE 6/8)自然產出正確 flags。

**未授權項(honest disclosure §一.8 + no-manual-fill)**:❌ committed snapshot FALSE flags 不 hand-patch(per 2026-05-31「以後都不該有你去補什麼資料」— 只改 writer + 重 build);❌ 無 git commit(需 §二.2 授權);❌ 無 retrain(eligibility flag 與 model artifacts 無關,trainer 不讀)。

**T_DC-29(Membership Eligibility-Flag Binding Treaty)— v0.16 NEW**:任一 universe-commit writer 寫 `core_universe_membership` core 列必須 explicit 設 4 個 eligibility/readiness 旗標與 gate verdict 一致(core 通過 → TRUE per 契約 2;quarantine/research → FALSE),**不得依賴 DEFAULT FALSE** 寫 core 列;全 universe writers 必須 mutually consistent。為 **T_DC-27 之 membership-layer 對應**(T_DC-27 治 feature-set-version NULL binding gap;T_DC-29 治 eligibility-flag DEFAULT-FALSE binding gap — 同構「writer 未 explicit 寫應寫欄位 → 落 default/NULL → 與 doctrine 矛盾」)。

**雙層治權鎖更新(v0.16)**:主憲章 §14.7-DC v0.16 + CLAUDE.md §一.13 v0.16 同次入憲(per T_DC-6)。T_DC accumulation:T_DC-1 ~ T_DC-29(v0.16 +T_DC-29)。

**證據基礎(本條入憲)**:用戶 2026-05-31 explicit「**把這次 ② writer fix + root cause 入憲(§14.7-DC 補一條 membership eligibility-flag binding,類比 T_DC-27)**」之直接入憲。Finding psql READ-ONLY(committed 398 core / train_eligible TRUE=0 / downstream_ready TRUE=0 / TRUE 僅 deprecated 200 + superseded 3724);root cause code-read(`core_universe_builder.py:2653-2658` 省略 + `core_universe_schema.py:133-136` DEFAULT FALSE + quarantine L2662-2669 明設 FALSE);SSOT 對照(`build_doctrine_gate_universe.py:274-281` 4×TRUE);corrective(L2653 補 4×TRUE,py_compile PASS)。**precedent**:首個 membership eligibility-flag binding 入憲;T_DC-27 之 membership-layer 對應;揭露治權契約 2 stated invariant 從未被 doctrine-native writer 實作。
...
**2. DB-Derived Schema SSOT(T_DJ-2/3)**:下游(builder/schema/audit)取 FinMind 表 schema 一律經 `data_schema.get_dataset_columns/get_dataset_keys`(查 DB `information_schema` = generic 所建),**不得**直接 index 退役之 `DATASET_REGISTRY[finmind_table]`。~~`DATASET_REGISTRY` 僅留 2 infra + FredData(FRED local-derived key generic 不可推導)~~ → **2026-06-08(cont)更新:`DATASET_REGISTRY` 徹底退役 → `INFRA_TABLE_SCHEMAS` 僅 2 infra log(`pipeline_execution_log`/`data_audit_log`,系統內部寫入非 API)**;**FredData 例外撤銷** —— `generic_schema.KEY_CANDIDATES` 將 `series_id` 前置於 `date`,generic 對 FRED 逐 series 樣本即正確推出 `(series_id,date)` PK(空 DB smoke 實證,原「不可推導」前提推翻);FRED 兩 writer(`sync_fred`→FredData、`fetch_fred_data`→fred_series)皆改 generic。下游取任何表 schema 一律經 `get_dataset_columns/get_dataset_keys`(infra 宣告 / 其餘 DB-derive),**不得** index `INFRA_TABLE_SCHEMAS[finmind/fred_table]` 或復辟任何 dataset 白名單。**§一.8 偏離揭露**:`LOCAL_DERIVED_COLUMNS` 保留(reconcile audit 仍用其跳 FRED series_id);fetch_fred per-day commit 放寬為 per-series;`audit_api_schema_compliance`(前 commit 本就 broken)+ `audit_supply_chain` 一併修;配套版本 generic_schema v1.2(字串下限 100→255,用戶 2026-06-08 directive)/ sync_engine v1.25 / data_schema v2.23 / fetch_fred v3.3。

**3. Data Dictionary Authority(T_DJ-4)**:通用 ingester 建之 **13 feature-input 表**(10 per-stock FinMind + TaiwanStockInfo 名冊 + FRED 2 表:`FredData` 4-series[`sync_fred`,5 欄] + `fred_series` 24-series[`fetch_fred_data`,3 欄,特徵庫 K-wave 來源];兩表 PK 皆 `(series_id,date)` generic 推導)完整字典已入主憲章 §14.7-DJ §二 + companion `reports/generic_ingester_data_dictionary_20260608.md`(2026-06-08(cont) 已修正 FRED 由「1 表 explicit-DDL」更正為「2 表 generic」);column 定義權威 = **API 欄位名 + `origin_name`(中文,長表)+ DB-verified 型別/樣本**,非 AI 臆測(§一.10);型別由實際值推導(generic 自動加寬)。FinMind 11 表欄位/大小寫 + FRED observation 欄位均經 2026-06-08 live-probe API 再驗證。任何特徵須以本字典理解 raw 來源意義。
