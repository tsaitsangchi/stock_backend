...
**Real example findings(2026-05-29 audit)**:
- ⚠️ `theme_strength`:scores from `THEME_KEYWORDS = {"半導體": 100, "生技": 95, ..., "汽車": 60}` hardcoded 14 個 entries → **AI 幻像 feature**(應移除 SPEC_43)
- ⚠️ `theme_is_semiconductor`:'半導體' string match → 機械式 binary,從 industry_category 真實 API value 衍生 / 邊界 case acceptable
